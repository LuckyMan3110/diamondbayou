...
class Admin extends Controller {
			...
			if ($action == 'delete') {
				...
				$data['onloadextraheader'] .= " $(\"#results\").flexigrid
									(
										...
										dataType: 'json',
										colModel : [
										{display: 'Orderid', name : 'orderid', width : 150, sortable : true, align: 'center'},
										{display: 'Lot', name : 'lot', width : 150, sortable : true, align: 'center'},
										{display: 'Quantity', name : 'quantity', width : 150, sortable : true, align: 'center'},
																			...
																			if($('.trSelected').length>0){
																			            if(confirm('Remove ' + $('.trSelected').length + ' rows?')){
																                                ...
																                                $.ajax({
																										   type: \"POST\",
																										   dataType: \"json\",
																										   url: \"" . config_item('base_url') . "admin/helixpriceStuller/delete\",
																										   data: \"items=\"+itemlist,
																										   success: function(data){
																										   	alert('Total Deleted rows: '+data.total);
																										    $(\"#results\").flexReload();
		...
		if ($this->isadminlogin()) {
			...
			$data['onloadextraheader'] .= " $(\"#results\").flexigrid
									(
										...
										dataType: 'json',
										colModel : [
										{display: 'Item Number', name : 'item_number', width : 100, sortable : true, align: 'center'},
                                                                                {display: 'Item ID', name : 'itemid', width : 50, sortable : true, align: 'left'},
                                                                                {display: 'Level1', name : 'level1', width : 150, sortable : true, align: 'center'},
										{display: 'Series', name : 'series', width : 180, sortable : true, align: 'left'},
										{display: 'Quality', name : 'quality', width : 180, sortable : true, align: 'left'},
										{display: 'Measurement', name : 'measurement', width : 90, sortable : true, align: 'left'},
                                                                                {display: 'Metal', name : 'metal', width : 90, sortable : true, align: 'left'},
										{display: 'Wholesale Price', name : 'wholesale_price', width : 60, sortable : true, align: 'left'},
										{display: 'Retail Price', name : 'retail_price', width : 50, sortable : true, align: 'left'}
										],
										 buttons : [ 				 								
	...
	function getstuller() {
		...
		foreach ($results['result'] as $row) {
			...
			$json .= ",'" . addslashes($row['level1']) . " '";
			$json .= ",'" . addslashes($row['series']) . " '";
			$json .= ",'" . addslashes($row['quality']) . " '";
			$json .= ",'" . addslashes($row['measurement']) . " '";
			$json .= ",'" . addslashes($row['metal']) . " '";
			...
			*/
			...
			$data['onloadextraheader'] .= " $(\"#results\").flexigrid
									(
										...
										{display: 'Item ID', name : 'itemid', width : 50, sortable : true, align: 'center'},
                                                                                {display: 'Description', name : 'description', width : 150, sortable : true, align: 'left'},
{display: 'Weight', name : 'weight', width : 150, sortable : true, align: 'center'},
										{display: 'Book', name : 'book', width : 180, sortable : true, align: 'left'},
                                                                                {display: 'Price', name : 'cost_40', width : 80, sortable : true, align: 'left'}
										...
										searchitems : [
										{display: 'Item Number', name : 'itemid', isdefault: true}
										],
										sortname: \"qg_id\",
										sortorder: \"asc\",
										usepager: true,
