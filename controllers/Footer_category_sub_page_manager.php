<?php
class Footer_category_sub_page_manager extends CI_Controller {
	private $totalEmails;
	...
	function index() {
		...
		if ($this->isadminlogin()) {
			...
			if(isset($_GET['delete_page_id'])){
				...
				$this->adminmodel->delete_any_table('content_sub_page', 'sub_page_id', $delete_page_id);
			...
			$where_csc_id						=  array('sub_page_id >' => '0');
			$data['get_category_list_count'] 	=  $this->adminmodel->count_any_table($where_csc_id, 'content_sub_page');
			$data['get_category_list']			=  $this->adminmodel->getdata_any_table_limit_order_where('content_sub_page', $where_csc_id, $limit, $offset, 'sub_page_id');

			
			$output = $this->load->view('admin/footer_category_sub_page_manager', $data, true);
		} else
	...
	function add() {
		...
		if ($this->isadminlogin()) {
			...
			if(isset($_POST['edit_page'])){
				...
				$where_count		=  array('title' => $this->input->post('title'), 'page_id' => $this->input->post('page_id'));
				$get_res_count 		=  $this->adminmodel->count_any_table($where_count, 'content_sub_page');

				if($get_res_count > 1){
					$data['msg'] 		= '<b>Sorry! This Category " '.$this->input->post('title').' " Already Used Another.';
					$data['msg_notify'] = 2;
				}else{
					$this->adminmodel->update_any_table($updateData, 'content_sub_page', 'sub_page_id', $edited_page_id);
		    ...
		    if(isset($_POST['add_page'])){
				...
				$insertdata['page_id']	    = $this->input->post('page_id');
				$insertdata['content']	    = $this->input->post('content');

				$insertdata['status']	    = 'Active';
				$insertdata['entry_date']	= date("Y-m-d");

				$where_count		        =  array('title' => $insertdata['title'], 'page_id' => $this->input->post('page_id'));
				$get_res_count 		        =  $this->adminmodel->count_any_table($where_count, 'content_sub_page');
				...
				if($get_res_count > 0){
					...
					$this->adminmodel->insert_into_any_table($insertdata, 'content_sub_page');
			...
			if(isset($_GET['edit_page_id'])){
				...
				$where_cat_id		    = array('sub_page_id' => $edit_page_id);
				$data['get_cat']        = $this->adminmodel->getdata_any_table_where($where_cat_id, 'content_sub_page');

			}
			
			$data['get_page_list']	= $this->adminmodel->getdata_any_table('content_man_page');
		    
			
			$output = $this->load->view('admin/add_footer_category_sub_page_manager', $data, true);
		} else
		$output = $this->load->view('admin/login', $data, true);
		$this->output($output, $data);
	}
