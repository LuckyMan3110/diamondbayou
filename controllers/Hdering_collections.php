<?php
class Hdering_collections extends CI_Controller {
    private $finish;
    ...
    function hear_cate_link($c_id=0) {
        ...
        if( !empty($cates3['cate_name']) ) {
            $cate_link .= '<li><a href="'.SITE_URL.'hdering_collections/heart_collection_cate/'.$cates3['id'].'">'.$cates3['cate_name'].'</a></li> > ';
        }
        if( !empty($cates2['cate_name']) ) {
            $cate_link .= '<li><a href="'.SITE_URL.'hdering_collections/heart_collection_cate/'.$cates2['id'].'">'.$cates2['cate_name'].'</a></li> > ';
        }
        if( !empty($cates1['cate_name']) ) {
            $cate_link .= '<li><a href="'.SITE_URL.'hdering_collections/heart_collection_cate/'.$cates1['id'].'">'.$cates1['cate_name'].'</a></li>';
        }
    ...
    function get_cate_list($gender='M') {
        ...
        foreach($men_cate as $rowmen) {
            ...
            if( !empty($cate_name) ) {
                $collection_cate .= '<li><a href="'.SITE_URL.'hdering_collections/heart-collection-listing/'.$rowmen['category'].'">'.$cate_name.'</a>';
            ...
            if( count($get_subcate) > 0 ) {
                ...
                foreach( $get_subcate as $rowm_scate ) {
                    ...
                    $collection_cate .= '<li><a href="'.SITE_URL.'hdering_collections/heart-collection-listing/'.$rowm_scate['subcategory'].'">'.$scate_name.'</a>';
                }
                $collection_cate .= '</ul>';
            }
    ...
    function heart_collection_cate($cates_id = 2) {
        ...
        if( $data['ring_categoies'] != 'false' ) {
            ...
            header('Location:' . SITE_URL . 'hdering_collections/ringlisting/' . $cates_id);
        }
    }
    ...
    function heart_bread_links($cid=0, $main='') {
        ...
        if( $cid > 0 ) {            
            ...
            if( empty($main) ) {
                $cate_link = ' > <li><a href="'.SITE_URL.'hdering_collections/heart-collection-listing/'.$cid.'">'.$cate_name.'</a></li>';
            } else {
    ...
    function heart_diamond_main_listing($start=0, $limit=4, $cols=2) {
        ...
        if( count($results['results']) > 0 ) {
            foreach( $results['results'] as $rowrings ) {
                $ringDetail = SITE_URL . 'hdering_collections/jewelry_ring_detail/' . $rowrings['id'];
    ...
    function collection_sort_options($id=0, $sort_value='') {
        ...
        $option_link = SITE_URL.'hdering_collections/heart-collection-listing/'.$id.'/?sort_value=';
        $view_sort_option = '';
    ...
    function get_item_cate_name($cate_id='') {
        $rowcate = $this->hderingitemsmodel->ebayitem_cate_row($cate_id);
        $cate_name = ( $cate_id == 'Band' ? 'Wedding Bands' : $cate_id );
        $category_id = ( $cate_id == 'Wedding Bands' ? 'Band' : $cate_id );
        $category_name = ( !empty($rowcate['category_name']) ? $rowcate['category_name'] : $cate_name );
        $cate_name_list = ' > <li><a href="'.SITE_URL.'hdering_collections/heart_collection_listing/'.$category_id.'">'.$category_name.'</a></li>';
        
        if( !empty($rowcate['sub_category_name']) ) {
            $cate_name_list .= ' > <li><a href="#">'.$rowcate['sub_category_name'].'</a></li>';
        }
    ...
    function heart_collection_listing($id='', $recent='') {
        ...
        $data['collection_cate_links'] = $this->get_collection_cate();
        $data['cate_name'] = $this->get_item_cate_name( $id );
        $data['collection_type'] = ( !empty($recent) ? 'Recently Purchased Products' : 'HD Rings' );
        ...
        $base_link = SITE_URL.'hdering_collections/heart-collection-listing/'.$id.'/?sort_value='.$sort_field;
	$config["base_url"]   = $base_link.'&pagelist='.$config["per_page"];
        ...
        $rowrings = $this->hderingitemsmodel->getCollectionJewelryList( $id, $start_from, $config["per_page"], $necklace, '', $sort_field, $recent );
        //print_ar($rowrings);
        ...
        $output = $this->load->view('engagementringstrial/hdering_collection_listing_view' , $data , true);
        $this->output($output, $data);
    }
    ...
    function get_price_field($metal='', $field='') {
        switch ($metal) {
            case '14k_gold':
                $field_name = 'g14_whsale_price';
                $metal_name = '14K White Gold';
                break;
            case '18k_gold':
                $field_name = 'g18_whsale_price';
                $metal_name = '18K White Gold';
                break;
            case 'platinum':
                $field_name = 'plat_whsale_price';
                $metal_name = 'Platinum';
                break;
                default :
                $field_name = 'g14_whsale_price';
        ...
        $returns['price_field'] = $field_name;
        $returns['metal_name'] = $metal_name;
        
        return ( empty($field) ? $field_name : $returns );
    }
    function jewelry_item_img($item_id=0) {
        ...
        foreach( $item_img as $rowimg ) {
            if( !empty($rowimg['image']) ) {
                $img_path_hd = HEART_MARKET . 'images/' .$rowimg['image'];
                //$img_path = SITE_URL . $rowimg['image'];
...
//                if( getimagesize($img_path) && count($item_image) == 0 ) {
//                    $item_image[] = $img_path;
//                    //break;
//                }
...
//                }
        ...
        if( count($item_image) > 0 ) {
            ...
            $ring_item_img[] = SITE_URL . 'img/no_img_found_icon.gif';
        }
    ...
    function get_metal_color_link($ring_style, $metal='', $carat='') {
        ...
        if( $id > 0 ) {
            $collection_link = SITE_URL . 'hdering_collections/collection-detail/'.$id;
        } else {
            $collection_link = '#';
        }
    ...
    function set_metal_links($ring_id=0, $ringstyle=0, $metal_type='', $metal_color='') {
        $metal_ic = $metal_ic = array(
                ...
                foreach( $metal_ic as $metalrow ) {
                    $metal_link = SITE_URL . 'hdering_collections/collection-detail/'.$ring_id . '/' . $metalrow['metal_type'];
                    
                    if( $metalrow['color'] == $metal_color && !empty($metal_color) ) { 
                        //$this->get_metal_color_link($ringstyle, $metalrow['metal'], $metalrow['carat']);
                        if( $i == 2 ) {
                        $metal_list .= '<li><a href="'.SITE_URL . 'hdering_collections/collection-detail/'.$ring_id.'/platinum"><img src="'.SITE_URL.'images/platinum_ic.jpg" /></a></li>';
                        }
    ...
    function getIndexDiamondList($ring_id=0, $shape='B', $ringColor='white', $item_metal='14k_gold', $start_limit=0) {
        $diamond_rows = $this->get_item_index_diamond_lists($ring_id, $shape, $ringColor, $item_metal, $start_limit);
        $page_limit = 500;
	...
	function getIndexDiamondListRignNew($ring_id=0, $shape='B', $ringColor='white', $item_metal='14k_gold', $start_limit=0, $ring_item_id=0, $ring_metal_type='PLAT',$stone=0) {
		$where_dev_index = array('price !=' => '', 'color !=' => '');
		$get_dev_index_info	= $this->catemodel->getdata_any_table_limit_order_by_where_like('dev_engagement_rings', $where_dev_index, '1500', '0', 'id', 'ASC', $stone, 'diamond_weight');

        $diamond_list = '';
        $check_show_data = 0;
        if( count($get_dev_index_info) > 0 ) {
			foreach( $get_dev_index_info as $k=>$rows ){
                $white_color_array      = array('D','E','F','G','H','I','J');
                $check_clarity_array    = array('FL','IF','SI1','VVS2','VVS1','VS1', 'VS2');
                //if( in_array($rows->color, $white_color_array) AND in_array($rows->clarity, $check_clarity_array) ){
                    $ebay_price = $rows->price * 1.10;
                    $detail_link = SITE_URL . 'selection/engagement-rings-detail/'.$ring_item_id.'/1/'.$ring_metal_type.'/?carat='. str_replace(" Carat"," ",$rows->diamond_weight) .'&dev_index_id='. $rows->id;
                    $diamond_color = check_empty($rows->color, '');
                    $diamond_list .= '<tr class="'.$k.'">
						<td class="s-'.$k.'">ROUND</td>
						<td class="dw-'.$k.'">'. str_replace(" Carat"," ",$rows->diamond_weight) .'</td>
						<td class="dc-'.$k.'">'.$diamond_color.'</td>
						<td class="dcl-'.$k.'">'.$rows->clarity.'</td>
						<td class="dp-'.$k.'">$'._nf($ebay_price, 2).'</td>
						<td class="dfinder"><a href="'.$detail_link.'"><img src="'.SITE_URL.'img/david_home/diamond_search/view_detail.jpg" alt="Details"></a></td>
					</tr>';
					$check_show_data++;
				//}
            }
        ...
        $page_limit = 500;
        $total_pages = round(count($get_dev_index_info) / $page_limit );
        $page_links = '<div><ul class="diamond_page_list">';
        $p = 0;
		for($i=1; $i <= $total_pages; $i++) {
			$start_from = $page_limit * $p;
    ...
    function getIndexDiamondListRign($ring_id=0, $shape='B', $ringColor='white', $item_metal='14k_gold', $start_limit=0, $ring_item_id=0, $ring_metal_type='PLAT',$stone=0) {
        $ring_cart1= '';
		if(!empty($_GET['ch_cart1'])){
			$ring_cart1 = $_GET['ch_cart1'];
		}
        $diamond_rows = $this->get_item_index_diamond_lists_ring($ring_id, $shape, $ringColor, $item_metal, $start_limit, $ring_item_id, $ring_metal_type,$ring_cart1);
    ...
    function get_item_index_diamond_lists($ring_id=0, $shape='B', $diamond_color='white', $item_metal='14k_gold', $start_limit=0) {
        $results = $this->hderingitemsmodel->get_item_index_diamond_rows($shape, $diamond_color, $start_limit); //print_ar($results);
        $diamond_shape = view_shape_value($shape_img, $shape); // custome helper
        
        $diamond_list = '';
        
        if( count($results['results']) > 0 ) {
            foreach( $results['results'] as $rows ) {
                $ebay_price = $rows['price'] * 1.10;
                $detail_link = SITE_URL . 'hdering_collections/collection-detail/'.$ring_id.'/'.$item_metal.'?ring_size=&diamond_lot=' . $rows['lot'];
                $diamond_color = check_empty($rows['color'], $rows['fcolor_value']);
                $diamond_list .= '<tr>
                            <td>'.$diamond_shape.'</td>
                            <td>'.$rows['carat'].'</td>
                            <td>'.$diamond_color.'</td>
                            ...
                            <td><a href="'.$detail_link.'" id="'.$rows['lot'].'"><img src="'.SITE_URL.'img/david_home/diamond_search/view_detail.jpg" alt="Details"></a></td>
                        </tr>';
            }
        }
        
        $returns['diamond_rows'] = $diamond_list;
        $returns['total'] = $results['total_rows'];
        
        return $returns;
    }
    
    function get_item_index_diamond_lists_ring($ring_id=0, $shape='B', $diamond_color='white', $item_metal='14k_gold', $start_limit=0, $ring_item_id=0, $ring_metal_type='PLAT',$stone=0) {
        $center_stone_solid_data = $stone;
        if($center_stone_solid_data){
            $where_dev_index	= array('fcolor_value =' => '', 'price !=' => '', 'color !=' => '', 'carat = ' => $center_stone_solid_data);
        }else{
            $where_dev_index	= array('fcolor_value =' => '', 'price !=' => '', 'color !=' => '', 'carat = ' => '0.25');
        }
        $ht = '';
        if(count($where_dev_index)>0){
            $i = 0;
            foreach($where_dev_index  as $k=>$v){
                $v = ($v!='')?$v:'""';
                $ht .= "(".$k."".$v.")";
                if($i<count($where_dev_index)-1){
                    $ht .=" and";
                }
                $i++;
            }
        ...
        $get_dev_index_info	= $this->catemodel->getdata_any_table_limit_order_by_where_like('dev_index', $where_dev_index, '1500', '0', 'uid', 'ASC', $center_stone_solid_data, 'carat');
        $diamond_list = '';
        $check_show_data = 0;
        if( count($get_dev_index_info) > 0 ) {
            foreach( $get_dev_index_info as $k=> $rows ) {
                ...
                $check_clarity_array    = array('FL','IF','SI1','VVS2','VVS1','VS1', 'VS2');
                if( in_array($rows->color, $white_color_array) AND in_array($rows->clarity, $check_clarity_array) ){
					$ebay_price = $rows->price;
					$detail_link = SITE_URL . 'selection/engagement_ring_detail/'.$ring_item_id.'/1/'.$ring_metal_type.'/?carat=' . $rows->carat.'&dev_index_id=' . $rows->uid;
					$diamond_color = check_empty($rows->color, $rows->fcolor_value);
					$diamond_list .= '<tr class="row-'.$k.'">
						<td class="shape-'.$k.'" data-Stock-n="'.$rows->Stock_n.'" data-shaped="'.$shape.'">'.$shape.'</td>
						<td class="carat-'.$k.'">'.$rows->carat.'</td>
						<td class="dc-'.$k.'">'.$diamond_color.'</td>
						<td class="dcl-'.$k.'">'.$rows->clarity.'</td>
						<td class="price-'.$k.'" data-p="'.$ebay_price.'">$'._nf($ebay_price, 2).'</td>
						<td><a class="dfinder" data-i="'.$k.'" href="'.$detail_link.'"><img src="'.SITE_URL.'img/david_home/diamond_search/view_detail.jpg" alt="Details"></a></td>
					</tr>';
					$check_show_data++;
				}
			}
    ...
    function diamond_carat_links($globalFields=[], $base_link='', $ring_diamond_color='') {
        $center_diamond = $this->get_diamond_spec( $globalFields ); //print_ar($center_diamond);
        $diamond_center_shape = diamond_shape_value( ucwords(strtolower($center_diamond['shape'])) ); // custome helper
        $diamond_lot1 = $this->hderingitemsmodel->get_heart_marekt_idex_diamond(0.25, $diamond_center_shape, $ring_diamond_color);
        $diamond_lot2 = $this->hderingitemsmodel->get_heart_marekt_idex_diamond(0.50, $diamond_center_shape, $ring_diamond_color);
        $diamond_lot3 = $this->hderingitemsmodel->get_heart_marekt_idex_diamond(0.75, $diamond_center_shape, $ring_diamond_color);
        $diamond_lot4 = $this->hderingitemsmodel->get_heart_marekt_idex_diamond(1.00, $diamond_center_shape, $ring_diamond_color);
        
        $returns['diamond_lot1'] = $base_link . $diamond_lot1;
        $returns['diamond_lot2'] = $base_link . $diamond_lot2;
        $returns['diamond_lot3'] = $base_link . $diamond_lot3;
        $returns['diamond_lot4'] = $base_link . $diamond_lot4;
        $returns['diamond_shape'] = $diamond_center_shape;
    ...
    function collection_detail($product_id=0, $metals_type='') {
        ...
        $data['title'] = 'Engagement Rings Detail | Hearts and Diamonds | Engagement Rings';
        ...
        //$imagesName = array();
        $metals_name = ( !empty($metals_type) ? $metals_type : '14k_gold' ) ;
        //$this->load->view('engagementringstrial/engagement_rings_details', $data);
        //$metal = '';
        //$rngsize = '';
        $ring_size = $this->input->get('ring_size', TRUE);
        $data['diamond_lot'] = $this->input->get('diamond_lot', TRUE);
        $data['item_ring_size'] = resetRingSize($ring_size);
        $data['shipping_policy'] = $this->get_page_content('shipping-policy');
        
        $rowsring = $this->hderingitemsmodel->getEbayItemDetailsViaID($product_id); //print_ar($rowsring);
        $jew_metal = set_jew_metal($rowsring['metal'], $rowsring['carat']);
        $metal_type = ( empty($metals_type) ? $jew_metal : $metals_type );
        $price_field = $this->get_price_field($metal_type, 'detail');
        $priceField = $price_field['price_field'];
        $data['diamond_detail'] = get_diamond_detail_row( $product_id, $data['diamond_lot'] );
        $data['metals_name'] = $price_field['metal_name'];
        $data['new_center_carat'] = ( !empty($data['diamond_detail']['carat']) ? $data['diamond_detail']['carat'] : $rowsring['new_center_carat'] );
        ...
        $data['cate_name'] = $this->get_item_cate_name( $rowsring['category_id'] );
        $data['category_name'] = $rowsring['category'];
        $categoryID = ( !empty($rowsring['sub_category']) ? $rowsring['sub_category'] : $rowsring['category'] );
        $data['bread_crumb_link'] = $this->heart_bread_crumb_links($categoryID);
        $data['comp_jew_title'] = $rowsring['category'];
        $metals_link = SITE_URL . 'hdering_collections/collection-detail/'.$product_id.'/';
        ...
        $data['heart_title'] = item_jew_title($rowsring, 0, 'Hearts and Diamonds', $price_field['metal_name'], $data['diamond_lot']); 
        /// item jewelry section helper
        $data['sales_price'] = set_ring_diamond_price( $rowsring, $data['diamond_lot'], $priceField );  // item jewelry section helper
        ...
        $data['item_desc'] = get_site_title().' Jewelry can be yours for $'._nf($rowsring['price'], 2).'.';
        ...
        $product_img = $this->jewelry_item_img( $rowsring['pid'] );
        $data['ringimg']   = $product_img[0];
        ...
        $data['popular_listings'] = $this->collectionSimilarLististing('Band', $product_id, 4, 'popular');
	...
	$data['finished_level'] = $this->finishlevel( $product_id, 'gold', 'options' );
        ...
        $data['main_detail_link'] = SITE_URL.'hdering_collections/collection-detail/'.$product_id.'/';
        $base_link = $data['main_detail_link'] . $metals_type.'/';
        $base_carat_link = $data['main_detail_link'] . $metals_type.'?ring_size='.$ring_size.'&diamond_lot=';
        ...
        if( count($ringresults) > 0 ) {
            foreach( $ringresults as $rowsize ) {
                ...
                $finger_size .= '<option value="'.$ring_size_link.'" '.selectedOption($rgsz, $ring_size).'>'.$rowsize['ring_size'].'</option>';
            }
        }
        ...
        $data['diamond_carats'] = $this->diamond_carat_links( $globalFields, $base_carat_link, $rowsring['idex_fw'] );
        //$data['diamond_rows'] = $this->get_item_index_diamond_lists( $data['diamond_carats']['diamond_shape'], $rowsring['idex_fw'], $base_carat_link);
        $data['load_center_diamond'] = 'setHDIndexDiamondList(\''.$product_id.'\', \''.$data['diamond_carats']['diamond_shape'].'\', \''.$rowsring['idex_fw'].'\', \''.$metals_name.'\', 0)';
        ...
        $data['recently_purchased'] = $this->recently_purchased_product($rowsring['category'], $product_id);
        ...
        //$data['item_details'] = $this->getJewelryItemDetails( $product_id, $rowsring['price'], $rowsring['new_price'] );
        $data['global_field_rows'] = $this->heart_items_detail_rows( $rowsring['global_fields'], $rowsring, '', $data['diamond_lot'] );
        $data['heart_item_details'] = $this->heart_other_fields_details( $rowsring, $price_field['price_field'], $price_field['metal_name'] );
        if( $rowsring['category'] == 'Ring' ) {
            $data['item_details_rows'] = $data['global_field_rows'] . $data['heart_item_details'];
        } else {
            $data['item_details_rows'] = $data['global_field_rows'] . $data['heart_item_details'];
        }
        $center_carat = ( !empty($data['diamond_lot']) ? $data['diamond_detail']['carat'] : $rowsring['new_center_carat'] );
        $data['total_carat'] = $center_carat + $rowsring['semi_mount_weight'];
        $data['ring_size'] = $ring_size;
        $view_page_name = ( $rowsring['category'] == 'Earrings' ? 'earrings_collections_detail_view' : 'hdering_collections_detail_view' );
        //$output = $this->load->view('engagementringstrial/collection_detail_view' , $data , true);
       //hdering_collections_detail_view echo $view_page_name; exit();
        $output = $this->load->view('engagementringstrial/'.$view_page_name , $data , true);
        $this->output($output, $data);
    }
    ...
    function item_list_view($specList=array(), $certNo='', $newCertNo='') {
        ...
        foreach( $specList as $label => $colValue ) {
            if( !empty($colValue) ) {
                ...
                $spec_lists .= '<div class="row detail'.$even_class.'">
                    <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> '.$label.' </span></div>
                    <div data-unique-id="width-column-1" class="column-1 "> <span> '.check_empty($colValue, 'NA') . $verify_cert_link .' </span> </div>
                </div>';   
    ...
    function heart_other_fields_details($details=[], $price_field='plat_whsale_price', $metal_name='') {
        //$item_total_carat = $details['new_center_carat'] + $details['semi_mount_weight'];
        
        $fields_list = array(
            'Metal' => $metal_name,
            'Total Diamonds' => $details['total_diam_pcs'],
            'Setting '.$metal_name.' Price' => '$' . _nf($details[$price_field])
        );
        $view_details = '';
        
        foreach( $fields_list as $labels => $cols_values ) {
            //if( !empty($cols_values) ) {
                $view_details .= '<div class="row detail">
                    <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> '.$labels.' </span></div>
                    <div data-unique-id="width-column-1" class="column-1 "> <span> '.check_empty($cols_values, 'NA') . '</span> </div>
                </div>';
                //}
        }
    ...
    function heart_items_detail_rows($globalFields='', $details=[], $right_hover='', $diamond_id='') {
        $fields_row_view = '';
        $allow_field = array('S-TYPE1', 'S-SHAPE1', 'S-SHAPE2');
        $diamond_lot_id = check_empty($diamond_id, $details['diamond_lot_id']);
        $diamondrow = $this->hderingitemsmodel->getIndexDiamondDetail($diamond_lot_id, 'lot'); //echo $diamond_lot_id; print_ar($diamondrow);
        $global_fields_list = array();
        ...
        if( !empty($globalFields) ) {
            ...
            foreach( $results as $cols_value ) {
                if( in_array($cols_value[0], $allow_field) ) {
                    ...
                    $global_fields_list[] = array('label' =>$field_lable, 'value' =>$cols_value[1] );
                    $fields_row_view .= '<div class="row detail">
                            <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> '.$field_lable.' </span></div>
                            <div data-unique-id="width-column-1" class="column-1 "> <span> '.check_empty($cols_value[1], 'NA') . '</span> </div>
                        </div>';
                }                
            }
        }
        $newCenterCarat = ( !empty($details['new_center_carat']) ? $details['new_center_carat'] : $details['center_carat'] );
        if( !empty($diamond_id) ) {
            $new_center_carat = $diamondrow['carat'];
            $cert_numb = $diamondrow['Cert_n'];
            $cert_label = $diamondrow['Cert_n'] . ' by GIA';
            $certficiate_no = $cert_label;
        } else {
            $results = itemJeweleryDetailsList( $details, $details['price'], $details['new_price'] ); /// item_jewelry_section helper
            $certficiate_no = $results['New Certificate #'];
            $new_center_carat = $newCenterCarat;
            $cert_numb = $details['certificate'];
            $cert_label = $details['index_diamond_cert'];
        }
        
        $total_carat = $new_center_carat + $details['semi_mount_weight'];
        
        $fields_row_view .= '<div class="row detail">
                        <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> Side Diamond Carats </span></div>
                        <div data-unique-id="width-column-1" class="column-1 "> <span> '. check_empty($details['semi_mount_weight'], 'NA') . '</span> </div>
                    </div>';
        $fields_row_view .= '<div class="row detail">
                        <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> Center Diamond Carat </span></div>
                        <div data-unique-id="width-column-1" class="column-1 "> <span> '. _nf($new_center_carat, 2) . '</span> </div>
                    </div>';
        $fields_row_view .= '<div class="row detail">
                        <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> Total Carat Weight : </span></div>
                        <div data-unique-id="width-column-1" class="column-1 "> <span> '. $total_carat . '</span> </div>
                    </div>';
        
        $verify_cert_link = getEbayRingItemCertVerifiyLink('New Certificate #', $cert_numb, $cert_label);   
        ///// item_jewelry_section helper
        $fields_row_view .= '<div class="row detail">
                            <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> Diamond Cert : </span></div>
                            <div data-unique-id="width-column-1" class="column-1 "> <span> '. $certficiate_no . $verify_cert_link . '</span> </div>
                        </div>';
        //$check_color = hditem_diamond_color_clarity($diamondrow['color'], $diamondrow['clarity']);  // custome helper
        if( $details['category'] != 'Band' ) {
        $fields_row_view .= '<div class="row detail">
                            <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> Diamond Color</span></div>
                            <div data-unique-id="width-column-1" class="column-1 "> <span> '. check_empty($diamondrow['color'], $diamondrow['fancy_color']) . '</span> </div>
                        </div>';
        $fields_row_view .= '<div class="row detail">
                            <div data-unique-id="width-column-0" class="column-0 tooltipcolumn"> <span> Diamond Clarity</span></div>
                            <div data-unique-id="width-column-1" class="column-1 "> <span> '. check_empty($diamondrow['clarity'], 'NA') . '</span> </div>
                        </div>';
        }
        
        if( empty($right_hover) ) {
            return $fields_row_view;
        } else {
            return $global_fields_list;
        }
    }
    ...
    function collection_ring_detail($product_id=0, $metal_type='14k_gold') {
        ...
        //$cate = $this->getCateName( $rowsring['catid'] );  
        $ringBoxDesc = $data['category_name'] . ' ' . $rowsring['metal_purity'];
        $data['ringtitle'] = $ringBoxDesc. ' Item ID: '. $rowsring['pid'];
	...
	$data['finished_level'] = $this->finishlevel( $product_id, 'gold', 'options' );
        ...
        $ringresults = $this->catemodel->getFingerSizeResult();
        $base_link = SITE_URL.'hdering_collections/collection-detail/'.$product_id.'/';
        $finger_size = '<option value="'.$base_link.'?ring_size=">Select Finger Size</option>';
    ...
    function heart_collection_viewtype_listing($cid=0, $start=0, $limit=4, $cols=2, $type='', $sroting='') {
        ...
        if( count($rowrings['results']) > 0 ) {
            foreach( $rowrings['results'] as $rows ) {
               $imageLink = $this->jewelry_item_img($rows['pid']);
               if( !empty($imageLink[0]) ) {
               $ringPrice = ( $rows['price'] > 0 ? '$ ' . _nf($rows['ring_price'], 2) : '' );
               $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rows['pid'];
               $popupLink = SITE_URL . 'hdering_collections/heart_collection_popup_detail/'.$rows['pid'] . '/?keepThis=true&TB_iframe=false&width=900&height=550&modal=true';
               ...
               $ring_listing .= '<div class="heart_list_viewbk">
                    <div class="col-sm-4 heart_img_view" onmouseover="show_collection_block(\''.$rows['pid'].'\')" onmouseout="hide_block(\''.$rows['pid'].'\')">
                        <div><a href="'.$detaiLink.'">
                           <div class="set_thumb_img similar_collection" id="thumb_'.$rows['pid'].'">
                            ' . $thumbs['thumbs'] . '
                            </div>
                            </a></div>
                    ...
                    <div class="col-sm-7 pull-right heart_content_section">
                        ...
                        <div class="col-sm-4 pull-right add_tocart_icon"><a href="#javascript" onclick="submitCartForm(\''.SITE_URL.'jewelleries/addtoshoppingcart/\', \''.$rows['pid'].'\')">Add to Cart</a></div>
                    </div>
                    <div class="clear"></div>
                </div>';               
               $i++;               
    ...
    //// Hearts and Arrows Collection jewelry rings
    function heart_collection_newarrivals_listing( $rowrings=array() ) { //print_ar($rowrings);
        ...
        if( count($rowrings) > 0 ) {
            foreach( $rowrings as $rows ) {
               $imageLink = $this->jewelry_item_img($rows['pid']);
               $global_fields = unserialize( $rows['global_fields'] );
               $diamond_id = $this->diamond_carat_links( $global_fields, '', $rows['idex_fw'] );
               //if( !empty($imageLink[0]) ) {
               $ringPrice = set_ring_diamond_price( $rows, $diamond_id ); // item jewelry section helper
               $ring_price_view = ( $ringPrice > 0 ? '$' . _nf($ringPrice, 2) : ' Please call' . CONTACT_NO . ' for price' );
               $retailPrice = $ringPrice * PRICE_PERCENT;
               if( $rows['diamond_lot_id'] > 0 && $rows['category'] != 'Band' ) {
                   $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rows['pid'].'/14k_gold?ring_size=&diamond_lot='.$diamond_id;
               } else {
                   $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rows['pid'].'/14k_gold?ring_size=';
               }
               
               $popupLink = SITE_URL . 'hdering_collections/heart_collection_popup_detail/'.$rows['pid'] . '/?keepThis=true&TB_iframe=false&width=900&height=550&modal=true';
               //$ringTitle = item_jew_title($rows); /// item jewelry section helper
               $ringTitle = item_jew_title($rows, 0, 'Hearts and Diamonds', '14K White Gold', $diamond_id);
               $rid = $rows['pid'];
               $popup_block .= $this->heart_collection_popup_detail($rows['pid']);
               $thumbs = $this->heart_collection_images($rows, 'Hearts and Diamonds Collection');
               
               $ring_listing .= '<li class="item">'
                       . '<div class="set_bk_height product-grid-quick" onmouseover="show_collection_block(\''.$rid.'\'); showQuickView('.$rid.')" onmouseout="hide_block(\''.$rid.'\'); hideQuickView('.$rid.')">'
                        . '<div><a href="'.$detaiLink.'">
                           <div class="set_thumb_img similar_collection" id="thumb_'.$rid.'">
                               '.$thumbs['thumbs'].'
                            </div>
                            </a></div>
                           ' . $this->collection_hover_block($rows, $ringPrice, $imageLink[0], $popupLink, $thumbs['total'], 'listing', '', $ringTitle) . '
    ...
    <div class="popup-prices">';
     ...
     <li>';
               ...
               $collection_listing_view .= '<div class="heart_list_viewbk" onmouseover="show_collection_block(\''.$rid.'\')" onmouseout="hide_block(\''.$rid.'\')">
                    <div class="col-sm-4 heart_img_view">
                        <div><a href="'.$detaiLink.'">
                           <div class="set_thumb_img similar_collection" id="thumb_'.$rows['pid'].'">
                            <img src="'.$imageLink[0].'" width="150" height="125" class="" alt="Hearts and Diamonds Collection" />
                            </div>
                            </a></div>
                    </div>
                    <div class="col-sm-7 pull-right heart_content_section">
        ...
        $collection_listing_view .= '<div class="col-sm-4 pull-right add_tocart_icon"><a href="#javascript" onclick="submitCartForm(\''.SITE_URL.'jewelleries/addtoshoppingcart/\', \''.$rid.'\')">Add to Cart</a></div>
                    </div>
                    <div class="clear"></div>
                </div>';
               //$this->db->query("UPDATE dev_jewelries set have_img = 'Y' WHERE pid = '{$rows['pid']}'");
               $i++;               
               //} /// end image check if
            } /// end foreach
        } else {
    ...
    function heart_diamond_collection_list($cid=0, $start=0, $limit=4, $cols=2, $type='') {
        ...
        if( count($rowrings['results']) > 0 ) {
            foreach( $rowrings['results'] as $rows ) {
               ...
               $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rows['pid'];
               $popupLink = SITE_URL . 'hdering_collections/heart_collection_popup_detail/'.$rows['pid'] . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
               $thumbs = $this->heart_collection_images($rows, 'Hearts and Diamonds Collection');
               $popup_block .= $this->heart_collection_popup_detail($rows['pid']);
               $ringImage = $this->set_collection_img($rows['image1'], $rows['item_sku'], $rows['different_imglist']);
               
               $ring_listing .= '<div class="'.$block_cols.' set_bk_height" onmouseover="show_collection_block(\''.$rows['pid'].'\')" onmouseout="hide_block(\''.$rows['pid'].'\')">'
                        . '<div><a href="'.$detaiLink.'">
    ...
    ///// hover block
    function collection_hover_block($rows=array(), $ringPrice=0, $ringImage='', $popupLink='', $total_img = 1, $listing='', $viewtype='', $ringTitle='') {
        $block_id = $rows['pid'];
        $form_name = 'form_'.$block_id;
        $ring_price_view = ( $ringPrice > 0 ? '$' . _nf($ringPrice, 2) : ' Please call' . CONTACT_NO . ' for price' );
        $collection_title = ( empty($listing) ? 'Hearts and Diamond' : $rows['item_title'] );
        if( empty($viewtype) ) {
            $hover_block = '<div class="item_info_view">
                               <div class="item_lable_style"><a href="'.SITE_URL.'hdering_collections/collection-detail/'.$rows['pid'].'">
                                   '. $collection_title .'<br>Hearts and Diamonds Collection<br></a></div>
            ...
            $hover_block .= '<div class="collection_hover_bk" id="'.$rows['pid'].'">
                           <div class="view_count">1/'.$total_img.'</div>
                           <div class="quick_view"><a href="Javascript:;" onclick="view_simple_popup(\''.$rows['pid'].'\')" title="Quick View'.$rows['pid'].'" rel="nofollow">Quick <br> View</a></div>
                          <div class="left_arrow_view"><a href="#javascript" onclick="button_previous(\'thumb_'.$rows['pid'].'\'); show_number_count(\''.$block_id.'\', \''.$total_img.'\', \'back\')"><img src="'.SITE_URL.'img/heart_diamond/left_arrow_icon.jpg"></a></div>
                    <div class="right_arrow_view"><a href="#javascript" onclick="button_next(\'thumb_'.$rows['pid'].'\'); show_number_count(\''.$block_id.'\', \''.$total_img.'\', \'\')"><img src="'.SITE_URL.'img/heart_diamond/right_arrow_icon.jpg" alt=""></a></div>
                                ...
                                <input type="hidden" name="prid" id="prid" value="'.$rows['pid'].'" />
   ...
   function product_cate_list_link($cname='') {
    ...
    function getProductThumb($rowaray=array(), $popup='') {
        $getRingsThumb = '';
        $itemTitle = $rowaray['category'].' ITEM ID: ' . $rowaray['item_id'];
        
	$collection_thumb = $this->jewelry_item_img( $rowaray['pid'] );
        
        foreach( $collection_thumb as $thumbimage ) {
    ...
    function get_collection_thumbs($rowaray=array(), $item_title='') {
        ...
        if( count($collection_thumb) > 0 ) {  
            foreach( $collection_thumb as $thoption ) {
                $thumbimage = $thoption; //SITE_URL.COLLECTION_FOLDER.$thoption;
                $active_class = ( $i == 1 ? 'active_view' : '' );

                if( getimagesize($thumbimage) ) {
                    $unique_id = 'bk_'.$i.'_'.$rowaray['pid'];
            ...
            $set_ring_thumb .= '<div class=""><img src="'.SITE_URL.'img/no_image_found.jpg" class="set_static_position" width="200" height="200" alt="Hearts and Diamonds Collection"></div>';
        }
    ...
    function ringSimilarItems($ring_cateid=0, $prod_id=0, $limit=5, $listing='') {
        ...
        if( count($rowsring) > 0 ) {
            foreach( $rowsring as $rowring ) {
               $detaiLink = SITE_URL . 'hdering_collections/jewelry_ring_detail/' . $rowring['id'];
            ...
            //if( getimagesize( $ringImageLink ) ) {
               ...
               $rid = $rowring['id'];
               $popupLink = SITE_URL . 'hdering_collections/heart_popup_detail/'.$rowring['id'] . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
    ...
    ///// rings popular items 
    function recently_purchased_product($category_id=0, $pid=0) {
        //$rowsring = $this->hderingitemsmodel->recently_purchased_products($category_id);
        $rowsring = $this->hderingitemsmodel->collection_similar_product($category_id, $pid, 4);
        $prod_list = '';
        ...
        if( count($rowsring) > 0 ) {
            $prod_list .= '<div class="recently_purchased">
                    ...
                    <a href="'.SITE_URL.'hdering_collections/heart-collection-listing/'.$category_id.'">View All Products</a></div>
                        <div class="rpurchase_products">';
            
            foreach( $rowsring as $rowring ) {
                $global_fields = unserialize( $rowring['global_fields'] );
                $diamond_id = $this->diamond_carat_links( $global_fields, '', $rowring['idex_fw'] );
                if( $rowring['diamond_lot_id'] > 0 && $rowring['category'] != 'Band' ) {
                    $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rowring['pid'].'/14k_gold?ring_size=&diamond_lot='.$diamond_id;
                } else {
                    $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rowring['pid'].'/14k_gold?ring_size=';
                }
                
                $ring_title = item_jew_title($rowring); /// item jewelry section helper
                $thumbs = $this->heart_collection_images($rowring, $ring_title, '', 'recent');
                $prod_list .= '<a href="'.$detaiLink.'">'.$thumbs['thumbs'].'</a>';
            }
            $prod_list .= '</div></div>';
        }
    ...
    function newArrivalHomeProducts() {
        $rowsring = $this->hderingitemsmodel->collection_similar_product('Ring', 0, 10);
        ...
        if( count($rowsring) > 0 ){
            foreach( $rowsring as $rowring ) {
               $global_fields = unserialize( $rowring['global_fields'] );
               $diamond_id = $this->diamond_carat_links( $global_fields );
               if( $rowring['diamond_lot_id'] > 0 && $rowring['category'] != 'Band' ) {
                   $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rowring['pid'].'/14k_gold?ring_size=&diamond_lot='.$diamond_id;
               } else {
                   $detaiLink = SITE_URL.'hdering_collections/collection-detail/'.$rowring['pid'].'/14k_gold?ring_size=';
               }
               $ring_ourprice = set_ring_diamond_price( $rowring, $diamond_id ); /// item jewelry sectoin helper
               $ring_title = item_jew_title($rowring); /// item jewelry section helper
               $thumbs = $this->heart_collection_images($rowring, $ring_title, '', 'recent');
               $prodRingPrice = ( $ring_ourprice > 0 ? '$ '._nf($ring_ourprice, 2) : '<div class="setprice_label">PLEASE CALL '.CONTACT_NO.' FOR PRICE</div>');
                
               $products_list .= '<div class="prod_list_cols">';
               $products_list .= '<div><a href="'.$detaiLink.'">'.$thumbs['thumbs'].'</a></div>';
               $products_list .= '<div class="set_prods_title">'.$ring_title.'</div>';
               $products_list .= '<div class="set_prod_price">'.$prodRingPrice.'</div>';
               $products_list .= '</div>';
            }
        }
        echo $products_list;
    ...
    function collectionSimilarLististing($cid=0, $prod_id=0, $limit=5, $type='similar') {
        $rowsring = $this->hderingitemsmodel->collection_similar_product($cid, $prod_id, $limit);
        ...
        if( count($rowsring) > 0 ) {
            foreach( $rowsring as $rowring ) {
               ...
               $ringImageLink = $this->jewelry_item_img($rowring['pid']);
               $ring_title = $this->heart_ring_title( $rowring );
               $ring_ourprice = set_ring_diamond_price( $rowring, $diamond_id ); /// item jewelry sectoin helper
               $detailDesc = 'Metal Purity ' . $rowring['metal_purity'] . ' Metal Color ' . $rowring['metal_color'];
               $prodRingPrice = ( $ring_ourprice > 0 ? '$ '._nf($ring_ourprice, 2) : '<div class="setprice_label">PLEASE CALL '.CONTACT_NO.' FOR PRICE</div>');
               $popupLink = SITE_URL . 'hdering_collections/heart_collection_popup_detail/'.$rowring['pid'] . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
               $popupBlock .= $this->heart_collection_popup_detail($rowring['pid']);
               $hover_id_bk = $type . '_' . $rowring['pid'];
               $thumbs = $this->heart_collection_images($rowring, $ring_title);
            $similar_rings .= '<div class="product_colsbk col-sm-3" onmouseover="show_collection_block(\''.$hover_id_bk.'\')" onmouseout="hide_block(\''.$hover_id_bk.'\')">
            ...
            if( !empty($rowring['product_type']) ) {
                ...
                $similar_rings .= '<div class="centerLabel"><a href="'.$detaiLink.'">This is '.$ring_title.'</a></div>
                        ...
                        <div class="setcolor_label">Item ID: '.$rowring['item_id'].'</div><br>
                        <div>'. $prodRingPrice .'</div>
                           ' . $this->collection_detail_hover_block($rowring, $ring_ourprice, $ringImageLink[0], $popupLink, $type, $thumbs['total']) . ' 
                    </div>';
            }
        }
    ...
    function collection_detail_hover_block($rowring=array(), $ring_ourprice=0, $ringImageLink='', $popupLink='', $type='', $total_img = 1) {
        $form_name = 'form_'.$rowring['pid'];
        $block_id = $type.'_'.$rowring['pid'];
        $collection = '<div class="collection_hover_bk collection_detail_hover" id="'.$block_id.'">
                    <div class="view_count">1/'.$total_img.'</div>
                    <div class="quick_view"><a href="Javascript:;" onclick="view_simple_popup(\''.$rowring['pid'].'\');" title="Quick View'.$rowring['pid'].'" rel="nofollow">Quick <br> View</a></div>
                    ...
                    <div class="item_info_view">
                     ...
                     <div class="addtocart_icon"><a href="#javascript" onClick="submitCartForm(\''.SITE_URL.'jewelleries/addtoshoppingcart/\', \''.$rowring['pid'].'\')">Add to Cart</a></div>
                 <form name="'.$form_name.'" id="'.$form_name.'" method="post">
                     ...
                     <input type="hidden" name="prodname" value="Jewelry Item ID: '.$rowring['item_id'].'" />
                     ...
                     <input type="hidden" name="prid" id="prid" value="'.$rowring['pid'].'" />
                     <input type="hidden" name="txt_ringtype" value="hdering_diamond_collection" />
                     <input type="hidden" name="txt_ringsize" value="" />
    ...
    function heart_collection_images($rows=array(), $ring_title='', $detail='', $recent='') {
        ...
        $collection_thumb = $this->jewelry_item_img($rows['pid']);
    ...
    function popularListings($ring_cateid=0, $prod_id=0, $limit=5, $listing='') {
        ...
        if( count($rowsring) > 0 ) {
            foreach( $rowsring as $rowring ) {
               ...
               $ring_ourprice = $rowring['gold_polished_1300'];
               //$saving_percent = ( 100 - ( ( $ring_ourprice / $retail_price ) * 100 ) );
               //$popupLink = SITE_URL . 'hdering_collections/heart_popup_detail/'.$rowring['id'] . '/?keepThis=true&TB_iframe=true&width=800&height=550&modal=false';
    ...
    //if( getimagesize( $ringsImg ) !== FALSE ) {
        ...
        $popupLink = SITE_URL . 'hdering_collections/heart_popup_detail/' . $rid . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
    ...
    function getSubCategoryLink( $cid=0 ) {
        ...
        if( count($ringresults) > 0 ) {
            foreach( $ringresults as $rowcate ) {
                $cateList .= '<li><a href="'.SITE_URL.'hdering_collections/heart_collection_cate/'.$rowcate['id'].'" title="'.$rowcate['cate_name'].'">'.$rowcate['cate_name'].'</a></li>';
            }            
        }
    ...
    function category_cols_list_view($cid=0) {
        ...
        if( count($ringresults) > 0 ) {
            foreach( $ringresults as $rowcate ) {
                $detaiLink = SITE_URL.'hdering_collections/heart_collection_cate/'.$rowcate['id'];
    ...
    function listingdetail($id=0, $popup='') {
        $rowsring = $this->hderingitemsmodel->getEbayItemDetailsViaID($id); //print_ar($rowsring);
        $global_field_list = $this->heart_items_detail_rows( $rowsring['global_fields'], $rowsring, 'rightHover' );
        //$item_detail_list = itemJeweleryDetailsList($rowsring, $rowsring['price']);  // item jewelry section helper
        $item_id = check_empty($rowsring['random_item_id'], $rowsring['item_id']);
...
<table id="item-tbl" class="data-table2" width="100%" cellspacing="0" cellpadding="0">
  <tbody>';
        $view_iteminfo .= '<tr class="first-row">
                        ...
                        <td style="color:#ff0000;font-weight:bold;">'.$item_id.'</td>
                      </tr>';
        
        foreach( $global_field_list as $item_label ) {
            if( !empty($item_label['value']) ) {
                $view_iteminfo .= '<tr class="first-row">
                                <th>'.$item_label['label'].'</th>
                                <td>'.check_empty($item_label['value'], 'NA').'</td>
                              </tr>';
            }
        }
        $total_carat = $rowsring['new_center_carat'] + $rowsring['semi_mount_weight'];
        $new_center_carat = ( !empty($rowsring['new_center_carat']) ? $rowsring['new_center_carat'] : $rowsring['center_carat'] );
        
        $item_detail_list = array(
            'Metal' => 'Platinum',
            'Total Diamonds' => $rowsring['total_diam_pcs'],
            'Side Diamond Carats' => $rowsring['semi_mount_weight'],
            'Center Diamond Carat' => _nf($new_center_carat, 2),
            'Total Carat Weight' => $total_carat,
            'Setting Platinum Price' => '$' . _nf($rowsring['plat_whsale_price'])
        );
        
        foreach( $item_detail_list as $list_label => $list_value ) {
            $view_iteminfo .= '<tr class="first-row">
                                <th style="width:40%;">'.$list_label.'</th>
                                <td>'.check_empty($list_value, 'NA').'</td>
                              </tr>';
        }
    ...
    function heart_collection_popup_detail($id=0) {
       $data = $this->getCommonData();
       $rowsring = $this->hderingitemsmodel->getEbayItemDetailsViaID($id);
        $data['category_name'] = $rowsring['category'];
        $data['rowsring'] = $rowsring;
        $sales_price = set_ring_diamond_price( $rowsring );  // item jewelry section helper
        $retail_price = $sales_price * PRICE_PERCENT;
        $saving_percent = _nf( ( 100 - ( ( $sales_price / $retail_price ) * 100 ) ), 2 );
        $data['buynow_link'] = htmlspecialchars(SITE_URL.'jewelleries/addtoshoppingcart/');
        $productName = item_jew_title($rowsring); /// item jewelry section helper
        $data['item_desc'] = $productName.' Jewelry can be yours for $'._nf($sales_price, 2).'.';

        //$cate = $this->getCateName( $rowsring['catid'] );  
        $ringBoxDesc = $data['category_name'] . ' ' . $rowsring['metal_purity'];
        $ring_title = $ringBoxDesc. ' Item ID: '. $rowsring['pid'];
        $product_img = $this->jewelry_item_img( $rowsring['pid'] );
        $item_id = check_empty($rowsring['random_item_id'], $rowsring['item_id']);
        $ringimg   = $product_img[0];
       ...
       $ring_detail_bk .= '<div class="col-sm-5">
                  <div class="product-image product-image-zoom zoomright" id="ringsthumb_view">
                    <div class="set_thumb_img" id="popup_'.$rowsring['pid'].'">                                
                            <div class="" id="show_thumb_view"></div>
                                  '.$product_thumbs['thumbs'].'
                            </div>
                    <div class="left_arrow_view"><a href="#javascript" onclick="button_previous(\'popup_'.$rowsring['pid'].'\')"><img src="'.SITE_URL.'img/heart_diamond/left_arrow_view_ic.jpg" alt="" /></a></div>
        <div class="right_arrow_view"><a href="#javascript" onclick="button_next(\'popup_'.$rowsring['pid'].'\')"><img src="'.SITE_URL.'img/heart_diamond/right_arrow_view_ic.jpg" alt="" /></a></div>
                </div>
                <div class="more-views"></div>
              </div>';
       $ring_detail_bk .= '<form id="form_'.$rowsring['pid'].'" action="" method="">
           <div class="col-sm-6 pull-right">
          <div class="price-box">
              <div class="product-name"><h1>'.$ring_title.'</h1></div>
              <p class="old-price fb-old-price f-fix"> 
                  <span class="price-label">Item Code:</span> <span>'.$item_id.'</span> 
              </p>
              <p class="old-price fb-old-price f-fix"> 
                  <span class="price-label">Item Style:</span> <span>'.$rowsring['category'].'</span> 
              </p>';
            if($retail_price > 0 ) {
            $ring_detail_bk .= '<p class="old-price"> <span class="retail-label">Retail Price:</span> <span class="price" id="old-price-'.$rowsring['pid'].'">$'._nf($retail_price, 2).'</span></p>
            <p class="old-price"> <span class="ourprice-label">Our Price:</span> <span class="new-price" id="product-price-'.$rowsring['pid'].'">$'._nf($sales_price, 2).'</span> <span class="special-price-label" id="old_price">(Savings:'.$saving_percent.'%)</span> <span id="google_discount"></span> <span id="twitter_discount"></span> <span id="facebook_discount"></span> <span id="pinterest_discount"></span> <span id="total_price"></span> 
          ...
          <div class="product-options-bottom">
            <div class="add-to-cart-1">
                ...
                <input type="hidden" name="3ez_price" value="" />
                <input type="hidden" name="5ez_price" value="" />
                <input type="hidden" name="main_price" value="'.$sales_price.'" />
                <input type="hidden" name="price" value="'.$sales_price.'" />
                <input type="hidden" name="vender" value="hdering_diamond_collection">
                ...
                <input type="hidden" name="prid" id="prid" value="'.$rowsring['pid'].'" />
                ...
                <div>
                    <a href="#javascript" onclick="add_to_cart_form(\''.SITE_URL.'jewelleries/addtoshoppingcart/\', \''.$rowsring['pid'].'\')" class="add_to_cart_btn" id="addtoshopping">Add To Cart</a>
                </div>
            </div>
          </div>
          <br clear="all">
    ...
    function hdring_search_result() {
        $data = $this->getCommonData(); 
        $data['title'] = 'Hearts and Diamonds Collection';
        $output = $this->load->view('engagementringstrial/hdring_search_result_view' , $data , true);
        $this->output($output , $data);		
    }
    //// get collection rings results
    function getCollectionRings($category_id=0, $page_limit=9) {
        ...
        if( count($results) > 0 ) {
            foreach( $results as $rows ) {
                $detailPageLink = SITE_URL . 'hdering_collections/collection_ring_detail/'.$rows['pid'];
