<?php
class Heartdiamond extends CI_Controller {
    private $finish;
    ...
    function hear_cate_link($c_id=0) {
        ...
        if( !empty($cates3['cate_name']) ) {
            $cate_link .= '<li><a href="'.SITE_URL.'heartdiamond/heart_collection_cate/'.$cates3['id'].'">'.$cates3['cate_name'].'</a></li> > ';
        }
        if( !empty($cates2['cate_name']) ) {
            $cate_link .= '<li><a href="'.SITE_URL.'heartdiamond/heart_collection_cate/'.$cates2['id'].'">'.$cates2['cate_name'].'</a></li> > ';
        }
        if( !empty($cates1['cate_name']) ) {
            $cate_link .= '<li><a href="'.SITE_URL.'heartdiamond/heart_collection_cate/'.$cates1['id'].'">'.$cates1['cate_name'].'</a></li>';
        }
    ...
    function get_cate_list($gender='M') {
        ...
        foreach($men_cate as $rowmen) {
            ...
            if( !empty($cate_name) ) {
                $collection_cate .= '<li><a href="'.SITE_URL.'heartdiamond/heart-collection-listing/'.$rowmen['category'].'">'.$cate_name.'</a>';
            ...
            if( count($get_subcate) > 0 ) {
                ...
                foreach( $get_subcate as $rowm_scate ) {
                    ...
                    $collection_cate .= '<li><a href="'.SITE_URL.'heartdiamond/heart-collection-listing/'.$rowm_scate['subcategory'].'">'.$scate_name.'</a>';
                }
                $collection_cate .= '</ul>';
            }
...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">';
        ...
        if( $data['ring_categoies'] != 'false' ) {
            ...
            header('Location:' . SITE_URL . 'heartdiamond/ringlisting/' . $cates_id);
        }
    }
	...
	$data['cate_links'] = $this->hear_cate_link($category_id);
        ...
        $base_link = SITE_URL.'heartdiamond/ringlisting/'.$category_id.'/?sort='.$sort_option;
	$config["base_url"]   = $base_link.'&pagelist='.$config["per_page"];
        ...
        $sort_option_link = SITE_URL . 'heartdiamond/ringlisting/'.$category_id.'/?pagelist='.$config["per_page"].'&per_page='.$startFrom.'&sort=';
    ...
    function heart_bread_links($cid=0, $main='') {
        ...
        if( $cid > 0 ) {            
            ...
            if( empty($main) ) {
                $cate_link = ' > <li><a href="'.SITE_URL.'heartdiamond/heart-collection-listing/'.$cid.'">'.$cate_name.'</a></li>';
            } else {
	...
	$data['collection_cate_links'] = $this->get_collection_cate();
        ...
        $data['rings_listing_3'] = $this->heart_diamond_collection_list( 3292598018, 0, 3, 6 );   //// rings  Note: 4th-parameter tells the col-sm-6, etc
...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">';
    ...
    function heart_diamond_main_listing($start=0, $limit=4, $cols=2) {
        ...
        if( count($results['results']) > 0 ) {
            foreach( $results['results'] as $rowrings ) {
                $ringDetail = SITE_URL . 'heartdiamond/jewelry_ring_detail/' . $rowrings['id'];
    ...
    function collection_sort_options($id=0, $sort_value='') {
        ...
        $option_link = SITE_URL.'heartdiamond/heart-collection-listing/'.$id.'/?sort_value=';
        $view_sort_option = '';
    ...
    function heart_collection_listing($id='', $recent='') {
        ...
        $base_link = SITE_URL.'heartdiamond/heart-collection-listing/'.$id.'/?sort_value='.$sort_field;
	    $config["base_url"]   = $base_link.'&pagelist='.$config["per_page"];
        $config['num_tag_open'] = '<li>';
        $config['num_tag_close'] = '</li>';
        $config['cur_tag_open'] = '<li><a class="active" href="#">';
    ...
    function get_metal_color_link($ring_style, $metal='', $carat='') {
        ...
        if( $id > 0 ) {
            $collection_link = SITE_URL . 'heartdiamond/collection-detail/'.$id;
        } else {
            $collection_link = '#';
        }
    ...
    function set_metal_links($ring_id=0, $ringstyle=0, $metal_type='', $metal_color='') {
        $metal_ic = $metal_ic = array(
                ...
                foreach( $metal_ic as $metalrow ) {
                    $metal_link = SITE_URL . 'heartdiamond/collection-detail/'.$ring_id . '/' . $metalrow['metal_type'];
                    
                    if( $metalrow['color'] == $metal_color && !empty($metal_color) ) { 
                        //$this->get_metal_color_link($ringstyle, $metalrow['metal'], $metalrow['carat']);
                        if( $i == 2 ) {
                        $metal_list .= '<li><a href="'.SITE_URL . 'heartdiamond/collection-detail/'.$ring_id.'/platinum"><img src="'.SITE_URL.'images/platinum_ic.jpg" /></a></li>';
                        }
...
//                );
    ...
    function heart_collection_detail($product_id=0, $metals_type='') {
        ...
        $metals_link = SITE_URL . 'heartdiamond/collection-detail/'.$product_id.'/';
	...
	$data['finished_level'] = $this->finishlevel( $product_id, 'gold', 'options' );
        ...
        $base_link = SITE_URL.'heartdiamond/collection-detail/'.$product_id.'/';
    ...
    function heart_collection_viewtype_listing($cid=0, $start=0, $limit=4, $cols=2, $type='', $sroting='') {
        ...
        if( count($rowrings['results']) > 0 ) {
            foreach( $rowrings['results'] as $rows ) {
               ...
               $detaiLink = SITE_URL.'heartdiamond/collection-detail/'.$rows['stock_number'];
               $popupLink = SITE_URL . 'heartdiamond/heart_collection_popup_detail/'.$rows['stock_number'] . '/?keepThis=true&TB_iframe=false&width=900&height=550&modal=true';
     ...
     <li>';
               ...
               $collection_listing_view .= '<div class="heart_list_viewbk" onmouseover="show_collection_block(\''.$rows['stock_number'].'\')" onmouseout="hide_block(\''.$rows['stock_number'].'\')">
                    <div class="col-sm-4 heart_img_view">
                        <div><a href="'.$detaiLink.'">
                           <div class="set_thumb_img similar_collection" id="thumb_'.$rows['stock_number'].'">
                            <img src="'.$ringImage.'" width="150" height="125" class="" alt="'.SITES_NAME.'" />
                            </div>
                            </a></div>
                    </div>
                    <div class="col-sm-7 pull-right heart_content_section">
    ...
    function heart_diamond_collection_list($cid=0, $start=0, $limit=4, $cols=2, $type='') {
        ...
        if( count($rowrings['results']) > 0 ) {
            foreach( $rowrings['results'] as $rows ) {
               ...
               $detaiLink = SITE_URL.'heartdiamond/collection-detail/'.$rows['stock_number'];
               $popupLink = SITE_URL . 'heartdiamond/heart_collection_popup_detail/'.$rows['stock_number'] . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
               $thumbs = $this->heart_collection_images($rows, SITES_NAME);
               $popup_block .= $this->heart_collection_popup_detail($rows['stock_number']);
               $ringImage = $this->set_collection_img($rows['image1'], $rows['item_sku'], $rows['different_imglist']);
    ...
    function collection_hover_block($rows=array(), $ringPrice=0, $ringImage='', $popupLink='', $total_img = 1, $listing='', $viewtype='') {
        ...
        if( empty($viewtype) ) {
            $hover_block = '<div class="item_info_view">
                               <div class="item_lable_style"><a href="'.SITE_URL.'heartdiamond/collection-detail/'.$rows['stock_number'].'">
            ...
            $hover_block .= '<div class="collection_hover_bk" id="'.$rows['stock_number'].'">
                    ...
                    <div class="right_arrow_view"><a href="#javascript" onclick="button_next(\'thumb_'.$rows['stock_number'].'\'); show_number_count(\''.$block_id.'\', \''.$total_img.'\', \'\')"><img src="'.SITE_URL.'img/heart_diamond/right_arrow_icon.jpg" alt=""></a></div>
                                ...
                                <input type="hidden" name="vendors_name" value="'.SITES_NAME.'" />
   ...
   function product_cate_list_link($cname='') {
	...
	$data['finished_level'] = $this->finishlevel( $product_id, 'gold', 'options' );
        ...
        $data['extra_headers'] .= '<script language="javascript" src="'.SITE_URL.'js/jquery.popup.js" type="text/javascript"></script>';
        $ringresults = $this->catemodel->getFingerSizeResult();
        $base_link = SITE_URL.'heartdiamond/jewelry_ring_detail/'.$product_id.'/';
        $finger_size = '<option value="'.$base_link.'?ring_size=">Select Finger Size</option>';
    ...
    function ringSimilarItems($ring_cateid=0, $prod_id=0, $limit=5, $listing='') {
        ...
        if( count($rowsring) > 0 ) {
            foreach( $rowsring as $rowring ) {
               $detaiLink = SITE_URL . 'heartdiamond/jewelry_ring_detail/' . $rowring['id'];
            ...
            //if( getimagesize( $ringImageLink ) ) {
               ...
               $rid = $rowring['id'];
               $popupLink = SITE_URL . 'heartdiamond/heart_popup_detail/'.$rowring['id'] . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
    ...
    function recently_purchased_product($category_id=0) {
        ...
        if( count($rowsring) > 0 ) {
            $prod_list .= '<div class="recently_purchased">
                    ...
                    <a href="'.SITE_URL.'heartdiamond/heart-collection-listing/newarrivals/'.$category_id.'">View All Products</a></div>
            ...
            foreach( $rowsring as $rowring ) {
                ...
                $prod_list .= '<a href="'.SITE_URL.'heartdiamond/collection-detail/'.$rowring['stock_number'].'">'.$thumbs['thumbs'].'</a>';
            }
            $prod_list .= '</div></div>';
        }
    ...
    function collectionSimilarLististing($cid=0, $prod_id=0, $limit=5, $type='similar') {
        ...
        if( count($rowsring) > 0 ) {
            foreach( $rowsring as $rowring ) {
               $detaiLink = SITE_URL . 'heartdiamond/collection-detail/' . $rowring['stock_number'];
               ...
               $popupLink = SITE_URL . 'heartdiamond/heart_collection_popup_detail/'.$rowring['stock_number'] . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
    ...
    function popularListings($ring_cateid=0, $prod_id=0, $limit=5, $listing='') {
        ...
        if( count($rowsring) > 0 ) {
            foreach( $rowsring as $rowring ) {
               ...
               $ring_ourprice = $rowring['gold_polished_1300'];
               //$saving_percent = ( 100 - ( ( $ring_ourprice / $retail_price ) * 100 ) );
               //$popupLink = SITE_URL . 'heartdiamond/heart_popup_detail/'.$rowring['id'] . '/?keepThis=true&TB_iframe=true&width=800&height=550&modal=false';
    ...
    //if( getimagesize( $ringsImg ) !== FALSE ) {
        ...
        $popupLink = SITE_URL . 'heartdiamond/heart_popup_detail/' . $rid . '/?keepThis=true&TB_iframe=true&width=900&height=550&modal=false';
    ...
    function getSubCategoryLink( $cid=0 ) {
        ...
        if( count($ringresults) > 0 ) {
            foreach( $ringresults as $rowcate ) {
                $cateList .= '<li><a href="'.SITE_URL.'heartdiamond/heart_collection_cate/'.$rowcate['id'].'" title="'.$rowcate['cate_name'].'">'.$rowcate['cate_name'].'</a></li>';
            }            
        }
    ...
    function category_cols_list_view($cid=0) {
        ...
        if( count($ringresults) > 0 ) {
            foreach( $ringresults as $rowcate ) {
                $detaiLink = SITE_URL.'heartdiamond/heart_collection_cate/'.$rowcate['id'];
...
<table id="item-tbl" class="data-table2" width="100%" cellspacing="0" cellpadding="0">
  <tbody>';
                     $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Item Code</th>
                        <td style="font-weight:bold;">'.$rowsring['stock_real_number'].'</td>
                      </tr>';
                      
                      $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Category Type</th>
                        <td style="font-weight:bold;">'.$dev_ebaycategories_data2['0']->category_name.'</td>
                      </tr>';
                      
                      $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Category Sub Type</th>
                        <td style="font-weight:bold;">'.$dev_ebaycategories_data['0']->category_name.'</td>
                      </tr>';
                      
                      $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Style</th>
                        <td style="font-weight:bold;">'.$rowsring['item_title'].'</td>
                      </tr>';
        
        foreach( $item_detail_list as $item_label => $item_value ) {
            if( !empty($item_value) ) {
                $view_iteminfo .= '<tr class="first-row">
                                <th style="width:40%;" style="font-weight:bold;">'.$item_label.'</th>
                                <td style="font-weight:bold;">'.$item_value.'</td>
                              </tr>';
            }
        }
 ...
 $view_iteminfo .= '</tbody></table>';
    ...
    ////// get product detail
    function listingdetail_watches($id=0, $popup='') {
        
        $rowsring = $this->davidsternmodel->getWatchCollectionDetail($id); //print_ar($rowsring);
...
<table id="item-tbl" class="data-table2" width="100%" cellspacing="0" cellpadding="0">
  <tbody>';
                     $view_iteminfo .= '<tr class="first-row">
                      ...
                      $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Category Type</th>
                        <td style="font-weight:bold;">Watches</td>
                      </tr>';
                      
                      $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Category Sub Type</th>
                        <td style="font-weight:bold;">'.$rowsring['brand'].'</td>
                      </tr>';
                      
                      $view_iteminfo .= '<tr class="first-row">
                        <th style="width:40%;font-weight:bold;">Style</th>
                        <td style="font-weight:bold;">'.$rowsring['SKU'].'</td>
                      </tr>';
    ...
    function getCollectionRings($category_id=0, $page_limit=9) {
        ...
        if( count($results) > 0 ) {
            foreach( $results as $rows ) {
                $detailPageLink = SITE_URL . 'heartdiamond/collection_ring_detail/'.$rows['stock_number'];
