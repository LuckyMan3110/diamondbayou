...
*/
class Jewelleriesm extends CI_Controller{
...
*/
	...
	function getmystullerfurwithsub($catname="",$maincat="",$status="")
	{
		...
		////// fetch data from dev_stuller table
	...
	function getstullerfur($catname="",$maincat,$status="")
	{
		...
		//echo $this->jewelleriesmodel->diamond_metal;exit;
...
$pth = substr(FCPATH,0,-10);
	...
	function getfinding_ringlist($start=0) {
		...
		//$result = $this->findingsmodel->getFindingsRingList($start, $config["per_page"]);
		...
		//$data['findingsresult'] = $result['result'];
...
//echo $this->uri->segment(3);
    ...
    function addtoshoppingcart()
	{
		...
		header("Location: " . config_item('base_url')."shoppingbasketm/mybasket");
		
		//redirect('/shoppingbasket/mybasket', 'refresh');
	}
	...
	function engagementQuality($catname="", $st='', $price_sort='asc', $limit=0, $uniquc='')
	{
		...
		//$data['sbcate_result'] = $this->jewelleriesmodel->getUniqueCategory($catname);
