...
class Store extends CI_Controller {
    ...
    function wedding_bands_diamond() {
        ...
        $data['page_links']   = get_pagination_links($per_page, $results['total'], 'store/wedding_bands_diamond/', $page_sort);
        $options_link = SITE_URL.'store/wedding_bands_diamond/?pagelist='.$per_page.'&per_page='.$perpage_no.'&sort=';
        ...
        if( count($results) > 0 ) {
            ...
            foreach( $results as $rows ) {
                ...
                $stuller_detail .= '<tr>
                                        ...
                                        <td><a href="'.SITE_URL.'store/wbands_diamond_detail/'.$s_id.'/'.$page_opt.'/'.$rows['id'].'">Select</a></td>
                                    </tr>';
            }
    ...
    function getFingerSizeOptions($product_id, $boptions='wb_diamond', $ring_size) {
        ...
        $base_link = SITE_URL.'store/wbands_diamond_detail/'.$product_id.'/'.$boptions.'/';
    ...
    function wedding_bands_ladies() {
        ...
        $startFrom = ( $perpage_no > 0 ? ( ( $perpage_no - 1 ) * $per_page ) : 0 );        
        $start_from =  ( $startFrom > 0 ? $startFrom : 1 );     
        $results = $this->stulleringsmodel->getWedingBandLadies($start_from, $per_page, $page_sort, $sort_value, $field);
        $data['page_links']   = get_pagination_links($per_page, $results['total'], 'store/wedding_bands_ladies/', $page_sort);
        
        $options_link = SITE_URL.'store/wedding_bands_ladies/?pagelist='.$per_page.'&per_page='.$perpage_no.'&sort=';
        $data['option_link'] = $this->sort_option_list($options_link, $page_sort);
    ...
    function wb_platinum() {
        ...
        $data['page_links']   = get_pagination_links($per_page, $results['total'], 'store/wb_platinum/', $page_sort);
        
        $options_link = SITE_URL.'store/wb_platinum/?pagelist='.$per_page.'&per_page='.$perpage_no.'&sort=';
        $data['option_link'] = $this->sort_option_list($options_link, $page_sort);
    ...
    function wedding_bands_platinum() {
        ...
        $start_from =  ( $startFrom > 0 ? $startFrom : 1 );
        $results = $this->stulleringsmodel->getWedingBandPlatinum($start_from, $config["per_page"], $sort_value, $field);
        
        $config["total_rows"] = $results['total'];
	    $config["base_url"]   = SITE_URL.'store/wedding_bands_platinum/?pagelist='.$config["per_page"];
...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">';
    ...
    function wedding_bands_mens() {
        ...
        $startFrom = ( $perpage_no > 0 ? ( ( $perpage_no - 1 ) * $config["per_page"] ) : 0 );        
        $start_from =  ( $startFrom > 0 ? $startFrom : 1 );
        $results = $this->stulleringsmodel->getWedingBandMens($start_from, $config["per_page"], $sort_value, $field);
        
        $config["total_rows"] = $results['total'];
	    $config["base_url"]   = SITE_URL.'store/wedding_bands_mens/?pagelist='.$config["per_page"];
        $config['num_tag_open'] = '<li>';
        $config['num_tag_close'] = '</li>';
        $config['cur_tag_open'] = '<li><a class="active" href="#">';
        $config['cur_tag_close'] = '</a></li>';
    ...
    function gemstone_earings() {
        ...
        $config["total_rows"] = $results['total'];
	$config["base_url"]   = SITE_URL.'store/gemstone_earings/?pagelist='.$config["per_page"];
        $config['num_tag_open'] = '<li>';
        $config['num_tag_close'] = '</li>';
        $config['cur_tag_open'] = '<li><a class="active" href="#">';
        $config['cur_tag_close'] = '</a></li>';
    ...
    function stud_earrings_view($stone_color='NA') {
        ...
        $page_link = SITE_URL.'store/stud_earrings_view/';
    ...
    function diamond_hoop_view() {
        ...
        $pageLink = SITE_URL.'store/diamond_hoop_view/';
    ...
    function getWedingBandsDiamondList($results=array(), $folder='wedding_bands_diamond', $page_option='') {
        ...
        if( count($results) > 0 ) {
            foreach ( $results as $rows ) {
                ...
                $detail_link = SITE_URL.'store/wbands_diamond_detail/'.$rows['id'].'/'.$page_option;
