...
class Stuller_fashion_jewelry extends CI_Controller {
	//private $totalEmails;
    ...
    function get_center_idex_diamond($id=0, $shape='B') {
		$rows = $this->stullerfashionmodel->updateIdexCenterDiamondToItem($id, $shape);
    }
    ...
    function stuller_fashionjew_listing($action = 'view', $type='HD', $filter_id=0, $id = '') {
		...
		if( isset($_GET['oauth_token']) AND isset($_GET['oauth_verifier']) ){
			...
			try {
				...
				echo ("<SCRIPT LANGUAGE='JavaScript'>
					...
					window.location.href='". SITE_URL ."stuller_fashion_jewelry/stuller_fashionjew_listing/view/".$links."';
                </SCRIPT>");
			}catch (OAuthException $e) {
				//print_ar('sdfsdf2222');
    		}
        ...
        if(!empty($item_id_list) && count($item_id_list) > 0) {
			$this->stullerfashionmodel->uploadItemPhotoGallery($item_id_list);
        }
		...
		if(isadminlogin()){
			...
			$url = SITE_URL . 'stuller_fashion_jewelry/getCollectionListings/' . $action . '/' . $link_strings;
			$data['action'] = $action;
			$data['onloadextraheader'] .= "  var actionview = '".$action."';
			var home_link = '".SITE_URL . 'stuller_fashion_jewelry/heart_collection_items/view/' . $link_strings."';
			var photo_gallery_link = '".SITE_URL . 'stuller_fashion_jewelry/heart_collection_items/photogallery/' . $link_strings."';
			function amazonGaleryBtn() {
				window.location = '" . SITE_URL . "stuller_fashion_jewelry/heart_collection_items/amazongallery/".$link_strings."';
			}
			...
			if(actionview == 'photogallery' || actionview == 'amazongallery') {
				$(\"#results\").flexigrid({   	 							
					...
					buttons : [	{name: 'Home', bclass: 'add', onpress : homeBtn}, {name: 'Delete', bclass: 'delete', onpress : test}, {name: 'Revise', bclass: 'add', onpress : test}, {name: 'Photo Gallery', bclass: 'add', onpress : galeryBtn}, {name: 'Amazon Gallery', bclass: 'add', onpress : amazonGaleryBtn}, {name: 'SubmitGallery', bclass: 'add', onpress : submitGalry},{separator: true}],
					searchitems : [
				...
				$(\"#results\").flexigrid({   	 							
					...
					buttons : [	{name: 'Home', bclass: 'add', onpress : homeBtn}, {name: 'Delete', bclass: 'delete', onpress : test}, {name: 'Revise', bclass: 'add', onpress : test}, {name: 'Photo Gallery', bclass: 'add', onpress : galeryBtn}, {name: 'Amazon Gallery', bclass: 'add', onpress : amazonGaleryBtn},{separator: true}],
					searchitems : [
						{display: 'Item Number', name : 'id', isdefault: true}
                    ], 		
					sortname: \"id\",
					sortorder: \"ASC\",
					usepager: true,
					title: '<h1 class=\"pageheader\">Stuller Fashion Jewelry Ebay Items</h1>',
					useRp: true,
					rp: 50,
					showTableToggleBtn: false,
					width:1020,
			...
			$data['homeLink'] = ''. SITE_URL .'stuller_fashion_jewelry/stuller_fashionjew_listing/view/'. $link_strings .'';
			...
			$output = $this->load->view('admin/stuller_fashionjew_admin_listing', $data, true);
    ...
    function stuller_items_detail($prod_id = 0, $type='') {
        ...
        if ( isadminlogin() ) {
            ...
            $rowsring = $this->stullerfashionmodel->getStullerItemDetailViaID($prod_id); //print_ar($rowsring);
            ...
            $data['retail_price'] = $rowsring['Price'] * RETAIL_PERCENT;
            $data['saving_percent'] = ( 100 - ( ( $rowsring['priceRetail'] / $data['retail_price'] ) * 100 ) );
            $data['item_details_list'] = stuller_item_details_rows( $rowsring );

                $output = $this->load->view('admin/stuller_fashionjew_item_detail', $data, true);
                output_page($output, $data); /// admin_common_func helper
        } else {
                $output = $this->load->view('admin/login', $data, true);
                output_page($output, $data); /// admin_common_func helper
    ...
    function download_hdrings_imgs() {
         $results = $this->stullerfashionmodel->getCollectionJewListing('HD', 0, '', 1, 200, 'id', 'DESC', '', 'id');
        $img_list = '';
        $i = 1;
    ...
    function jewelry_item_img($item_id=0) {
        $item_img = $this->stullerfashionmodel->get_item_imgs( $item_id );
    ...
    function item_img_view_type() {
        ...
        $results = $this->stullerfashionmodel->getCollectionJewListing('ERPHD', 0, '', 1, 1000);
        $item_id = '';
    ...
    function update_market_id($item_id=0, $item_field='', $field_id=0) {
        ...
        $this->stullerfashionmodel->updateMarketID($item_id, $item_field, $field_id);
	...
	function getCollectionListings($action='view', $view_type='HD', $filterID=0, $cate_id='') {
            ...
            $results = $this->stullerfashionmodel->getStullerItemRows($view_type, $filterID, $cate_id, $page, $rp, $sortname, $sortorder, $query, $qtype);
            ...
            foreach ($results['result'] as $row) {
                ...
                $detail_link = SITE_URL . "stuller_fashion_jewelry/stuller_items_detail/" . $row['id'];
                ...
                $imgrow = $this->stullerfashionmodel->getMoreItemImgList($row['id']);
                $item_profit = ''; //$this->getIRingItemProfit($row['diamond_lot_id'], $row['Price'], $row['Price'], $row['Price']);
                $diamond_row = $this->stullerfashionmodel->getIndexDiamondDetail($row['diamond_lot_id'], 'lot');
...
//                }
        ...
        if($action == 'photogallery') {
            ...
            $json .= ",'" . addslashes( getMarketMoreImgView($imgrow['amazon_img6']) . '<input type="file" name="amazon6_'.$pk_id.'" />' ) . "'";
                ...
                $json .= ",'" . addslashes($semi_label.' $' . _nf($row['Price'], 2)) . " <br>"
                        . "<input type=\"text\" name=\"price_box\" id=\"price_box_".$row['id']."\" onkeyup=\"update_jcarat_price(\'".$row['id']."\')\" class=\"set_carat_field\" value=\"".round($row['Price'], 2)."\" />'";
                ...
                $json .= ",'" . addslashes($row['Weight'] . '<br><b>Total Carat</b><br>' . $newTotalCarat) . ""
                        ...
                        . $this->get_video_model_block($row['id'], $row['embed_link'], $prod_name).""
                        . $this->get_similar_jewelry_list($row['id'], $item_shape, $row['Price'], $prod_name).""
    ...
    function getIRingItemProfit($diamondID=0, $newPrice=0, $ring_price=0, $orignalPrice=0) {
        ...
        if( $diamondID > 0 ) {
            $results = $this->stullerfashionmodel->getIndexDiamondDetail($diamondID, 'lot');
    ...
    function getCenterDiamondViaCaratSearch($item_id=0, $item_shape='', $itemPrice=0, $itemCarat=0, $minprice=0, $maxprice=0) {  
        ...
        $itemdetail = $this->stullerfashionmodel->getItemDetailViaID($item_id);
        
        $results = $this->stullerfashionmodel->getCenterDiamondFilterResults($_GET, $item_id, $item_shape, $itemPrice, $items_carat, $minprice, $maxprice, $mcarat, $maxcarat);
        $item_details = $this->stullerfashionmodel->getItemSpecification($item_id, 'dev_ebay_details');
    ...
    function get_similar_jewelry_list($id=0, $itemShape='', $item_price=0, $d_carat=0) {
        $results = $this->stullerfashionmodel->getSimilarItemLists($id, $itemShape, $item_price); //print_ar($results);
        $itemdetail = $this->stullerfashionmodel->getItemDetailViaID($id);
        $item_cate = urldecode($itemCate);
        $item_details = $this->stullerfashionmodel->getItemSpecification($id, 'dev_ebay_details');
        $cert_list = array('EGL', 'GIA');
    ...
    function changeItemDiamondInfo($itemID=0, $diamondID=0) {
        ...
        $this->stullerfashionmodel->updateItemDMInfo($itemID, $diamondID);
    ...
    function update_jitems_price($itemID=0, $items_price=0) {
        ...
        $this->stullerfashionmodel->updateJewelryItemPrice($itemID, $item_price);
    ...
    function update_jewelry_item($id=0) {
       ...
       $this->stullerfashionmodel->updateJewelryItemEmbedLink($id, $embed_value);
    }
    ...
    function update_jewcarat_value($id=0) {
       ...
       $this->stullerfashionmodel->updateJewItemCarat($id, $center_carat, $side_carat);
    }
    
    function update_item_newprice($id=0, $newprice=0) {

       $this->stullerfashionmodel->updateNewPrice($id, $newprice);
       
    }
    
    function updateJewelryItemIDWithRandomNumber() {
        $this->stullerfashionmodel->update_item_detail_itemid();
