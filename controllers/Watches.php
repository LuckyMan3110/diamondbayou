...
///// get the listingts of watches according to the category id
function watches_listings($ids = 0) {
    $idlist = urlencode($ids);
    $idlist = explode('_ids_', $idlist);
    $cid_list = implode("','", $idlist);
    ...
    $ringresults = $this->qualitymodel->qualityGoldProdList($cid_list, $startListing, $config["per_page"]);
    $config["total_rows"] = $ringresults['total'];
    
    if( count($ringresults['results']) > 0 ) {
        foreach( $ringresults['results'] as $rowatch ) 
        {
            $ringdetail_link = SITE_URL.'qualitygold/quality_detail/'.$rowatch['id'];
            $getRingPrice = $rowatch['price'];
            $viewRingImg = QUALITY_IMGS . $rowatch['small_image']; 
            $ringBoxDesc = cleanString($rowatch['title']);
            $str = 150;
            $ringBoxDesc = cleanString($rowatch['title']);
            $stullerTitle = ( strlen($ringBoxDesc) > $str ? substr($ringBoxDesc, $str).'...' : $ringBoxDesc );
            
            $watchPrice = ( $getRingPrice > 0 ? 'Price: $'.number_format($getRingPrice,0) : 'call 415 626.5035 for prices' );
            $view_listing .= '<div class="col-sm-4">';
            $view_listing .= '<div class="engagement-product">
        <div class="image-engagement">
        <div class="setringsbg_bk">'.$trialUserLogo.'</div>
        <a href="'.$ringdetail_link.'">&nbsp;<img src="'.$viewRingImg.'" alt="'.$ringBoxDesc.'" width="155" hight="144"></a>
        </div>
        <div class="dw-arrow"><img src="'.SITE_URL.'images/down-ar.png" align="Down Arrow" /></div>
        <div class="price-product ringprice-section">
          <p class="ring-engagement"><a href="'.$ringdetail_link.'">'.$stullerTitle.'</a></p>
          <div>'.$watchPrice.'</div>
          </div>
      </div>';
        ...
        $view_listing .= '<div>No Watch Listing found of this category</div>';
    }
...
function watches_settings($diamondid = true,$addoption = '',$sidestone1 = '', $sidestone2= '', $extraoption = '')
{
    ...
    $minprice = 3000; //$this->jewelrymodel->getMinPrice();
    $maxprice = 15000; //$this->jewelrymodel->getMaxPrice();
    ...
    $watches_result = $this->watchesmodel->getWatchList();
    $qgold_watches = $this->qualitymodel->qualityGoldCateList(2);
    $qgold_watch_list = '';

    $watch_block = '';

    if( count($watches_result) > 0 )
    {
        foreach($watches_result as $rowatch) {
            $viewWatchsDetail = SITE_URL.'watches/watches_detail/'.$rowatch['productID'];
            $watchThumb = ( getimagesize( SITE_URL.$rowatch['thumb'] ) ? SITE_URL.$rowatch['thumb'] : SITE_URL.'images/noimage_found.jpg' );

            $watch_block .= '<div class="watch_block">
                                <div><a href="'.$viewWatchsDetail.'"><img src="'.$watchThumb.'" width="125" alt="'.$rowatch['productName'].'" /></a></div>
                                <div><a href="">'.$rowwatc['productName'].'</a></div>
                                <div>Price: $'.$rowatch['price1'].'</div>
                                <div><a href="'.$viewWatchsDetail.'">View Detail</a></div>
                        </div>';
        }
    } else {
            $watch_block = '<div>No Watches Record Found Yet</div>';
    }
    
    ///// left category watch list
    if( count($qgold_watches) > 0 ) {
        foreach( $qgold_watches as $rowatches ) {
            $qgold_watch_list .= '<div><input name="qgold_watches" id="'.$rowatches['id'].'" type="checkbox" value="'.$rowatches['id'].'" onclick="getwatchresults()" value="Rolex"> <label for="'.$rowatches['id'].'">'.$rowatches['title'].'</div>';
        }
    }
    
    $data['watch_block'] = $watch_block;
    $data['qgold_watch_list'] = $qgold_watch_list;
	...
	///// get wathces detail
	function watches_detail($watch_id = '')
	{
		$data = $this->getCommonData();
		$data['title']   = 'Watches Ring Settings';
		$data['details'] = $this->watchesmodel->getWatchByProductId($watch_id);
		$watchImageLink = SITE_URL.$data['details']['thumb'];
		
		$data['watch_image'] = ( getimagesize($watchImageLink) ? $watchImageLink : SITE_URL.'images/no_image_found.jpg' );
		$data['watch_price'] = $data['details']['price1'];
		$data['similarWatches'] = '';
		$data['view_coments'] = '';
		$data['watch_id'] = $watch_id;
		$data['watch_type'] = ( $data['details']['gender'] == 'men' ? 'MALE' : $data['details']['gender'] );
		$data['prod_type'] = getWatchIdType($details['id_type']);
		$data['buynow_link'] = SITE_URL.'jewelleries/addtoshoppingcart/';
		...
		$output = $this->load->view('watches/watch_details_view' , $data , true);
		$this->output($output , $data, false,false);
