...
function view_order_details_customer_content($o_id='', $option='', $manfact = '', $xml_content='', $view_price='') {
	$CI = & get_instance();
	...
	if( count($rowitemdt) > 0 )	{
		foreach($rowitemdt as $row_itdt) {
			...
			switch($row_itdt['addoption']) {
				case 'addunique':
					...
					if($row_itdt['venderinfo'] == 'unique'){
						$image_url = SITE_URL.'scrapper/imgs/'.$rowrings_test['ImagePath'];
						$measurement = $rowrings_test['top_width'].'x'.$rowrings_test['bottom_width'].'x'.$rowrings_test['top_height'].'x'.$rowrings_test['bottom_height'];
					}else{
					...
					if($row_itdt['table_name'] == 'jewelry_unlimited_products'){
					...
					}/* elseif($row_itdt['table_name'] == 'dev_jewelries'){
						$rowjewelries = $CI->Catemodel->getjewelriesImages($row_itdt['lot']);
						if((strpos($rowjewelries->ImagePath, 'http://') !== false) || (strpos($rowjewelries->ImagePath, 'https://') !== false)){
							$image_url = $rowjewelries->ImagePath;
						}else{
							$image_url = config_item('base_url').$rowjewelries->ImagePath;
						}
						...
						if(!empty($rowimaged)){
							...
							if((strpos($rowimages->image1, 'http://') !== false) || (strpos($rowimages->image1, 'https://') !== false)){
								$image_url = $rowimages->image1;
							}else{
								if(file_exists(''.$rowimages->image1 .'')){
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><a href="'.$row_itdt['item_url'].'" target="_blank"><img src="'.$image_url.'" alt="'.$rowrings_test['name'].'" width="'.$imgwidth.'" /></a></div><br><a href="'.$row_itdt['item_url'].'" target="_blank">View Details</a>';

						$itemDetails .= $item_rings_features;
						if( empty($rowrings_test['style_group']) ){
							$rowrings_test_style_group = $stock_real_number;
						}else{
							$rowrings_test_style_group = $rowrings_test['style_group'];
						}

						if( empty($rowrings_test['category_name']) ){
				...
				case 'addwatch':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$image_url.'" alt="'.$itemName.'" width="'.$imgwidth.'" /></div></td>
						<td>'.$itemName.'</td>
						<td>'.$rowatch['upc'].'</td>
				...
				case 'heart_diamond_collection':
					...
					$image_url = setCollectionImgLink($rowrings_test['image1'], $rowrings_test['item_sku'], $rowrings_test['different_imglist']); //collections section helper
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$image_url.'" alt="'.$itemName.'" width="'.$imgwidth.'" /></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
						<td>'.$itemName.'</td>
				...
				case 'sterncolectionjewelry':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$image_url.'" alt="'.$ringBoxDesc.'" width="'.$imgwidth.'" /></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
						<td>'.$ringBoxDesc.'</td>
				...
				case 'qualityrings':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$image_url.'" alt="'.$rowrings_test['title'].'" width="'.$imgwidth.'" /></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
				...
				case 'stullerrings':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$image_url.'" alt="'.$rowrings_test['Description'].'" width="'.$imgwidth.'" /></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
					...
					$order_xml_content .= '<measurement>NA</measurement>';
					$order_xml_content .= '<name>'.$item_desc.'</name>';
					$order_xml_content .= '<description>'.$item_desc.'</description>';
					$order_xml_content .= $xml_item_features;
					$order_xml_content .= '<quantity>'.$row_itdt['quantity'].'</quantity>';
					$order_xml_content .= '<price>'.$rowrings_test['Price'].'</price>';
					$order_xml_content .= '<totalprice>'.$row_itdt['totalprice'].'</totalprice>';
				...
				case 'addjewelry':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><a href="'.$row_itdt['item_url'].'"><img src="'.$imgUrljw.'" alt="'.$testName.'" width="'.$imgwidth.'" /></a></div><br><a href="'.$row_itdt['item_url'].'">View Details</a>';
				...
				case 'wb_platinum':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><a href="'.$row_itdt['item_url'].'"><img src="'.$image_url.'" alt="'.$rowrings_test['name'].'" width="'.$imgwidth.'" /></a></div><br><a href="'.$row_itdt['item_url'].'">View Details</a>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
						<td>'.$itemName.'</td>
						<td>'.$row_itdt['stock_number'].'</td>
				...
				case 'hearts_diamond_collection':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$image_url.'" alt="'.$rowrings_test['name'].'" width="'.$imgwidth.'" /></div>';    
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
						<td>'.$itemName.'</td>
						<td>'.$dmdetails['lot'].'</td>
				...
				case 'addtoring':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><a href="'.config_item('base_url').'selection/engagement_rings_detail/'.$row_itdt['ringsetting'].'"><img src="'.$setting_imgurl.'" alt="'.$row_itdt['lot'].'" width="'.$imgwidth.'" /></a><br><a href="'.config_item('base_url').'selection/engagement_rings_detail/'.$row_itdt['ringsetting'].'">View Details</a></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
						<td>'.$ring_desc.'</td>
				...
				case 'toearring':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$vimage_urlink.'" alt="'.$stud_title.'" width="'.$imgwidth.'" /></div></td>
						<td>'.$stud_title.'
				...
				case 'addpendantsetings':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$pendant_imgurl.'" alt="'.$solitiar_title.'" width="'.$imgwidth.'" /></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
				...
				case 'tothreestone':
					...
					$itemDetails .= '<tr>
						<td>'.$r.'</td>
						<td><div><img src="'.$unique_ringimg.'" alt="'.$itemtitle_3stone.'" width="'.$imgwidth.'" /></div>';
						$itemDetails .= $item_rings_features;
						$itemDetails .= '</td>
						<td>'.$itemtitle_3stone.'
						<div>Center Stone: '.$diamond['lot'].'</div>
			...
			if( $itemOrder['salestax_percent'] > 0 ) {
				$itemDetails .= '<tr>
					<td colspan="6">Total Item Amount</td>
					<td>$'._nf($totalnet_amount,2).'</td>
					</tr><tr>
					<td colspan="6">Sales Tax '.$itemOrder['salestax_percent'].'%</td>
					<td>$'.number_format($orderamount['taxamount'], 2).'</td>
				</tr>';
			}
			$itemDetails .= '<tr>
				<td colspan="5" style="text-align:right;font-weight:bold">Total Order Amount</td>
