	...
	function adminmenuhtml($page = 'myaccount'){
  		...
  		$usermenu .= '<div class="menu_list" id="secondpane"> <!--Code for menu starts here-->
  					...
  					<p class="menu_head" id="sitemanage">Site Manage</p>
						...
						/*$usermenu .= '<a href="'.config_item('base_url').'admin/mount"';
					  				$usermenu .= ($page == 'mount') ?  'class="active">' : '>';
					  			    	$usermenu .= 'Semi Mount</a>';*/
						$usermenu .= '<a href="'.config_item('base_url').'admin/category"';
					  	$usermenu .= ($page == 'other') ?  'class="active">' : '>';
					  			    	$usermenu .= 'Other Module</a>';
...
.")";
 ...
 function getrolex( $page =1 , $rp = 10 ,$sortname = 'lot' ,$sortorder = 'desc' ,$query= '' , $qtype = 'lot' , $oid = ''){
	...
	function addWatchtoEbay($detail, $action='Add'){
		...
		$watchDetail .='<br><B>Our Price:</B>'.$detail['price2'] * 1.07;
		...
		$price = round($detail['price2'] * 1.07);
		$watchDetail .= '<div>
...
<img src="'.$watchImage.'" border="0" height="598" width="312"></font></td></tr></tbody></table></td></tr></table>
	';
		...
		//$requestArray['minimumbestofferprice']  = $detail['price2'] * 1.07;
...
//die('pp');
	...
	function addRingtoEbay($detail, $action='Add'){
		...
		$watchDetail .='<br><B>Yellow Gold Price:</B>'.round($detail['yellow_gold_price']*1.10);
		$watchDetail .='<br><B>Platinum Price:</B>'.round($detail['platinum_price']*1.10).'</p></div>';
		$storeImage = config_item('base_url').'images/upperbar02.jpg';
		...
		$colorImage = config_item('base_url').'images/Color_Profile.jpg';
		$price = round($detail['white_gold_price'] * 1.10);
		$retailPrice = round($price * 4.5);
		$yellow_gold_price = round($detail['yellow_gold_price'] * 1.10);
		$platinum_price = round($detail['platinum_price'] * 1.10);
		/*$watchDetail .='<div>
			<TABLE width=598 align=center border=0>
			<TBODY>
			<TR>
			...
			<TD width=509 height=22>&nbsp;White Gold Price:&nbsp;'.round($detail['white_gold_price'] * 1.10).'</TD></TR>
			<TR>
			<TD width=509 bgColor=silver height=18>&nbsp;Yellow Gold Price:&nbsp;'.round($detail['yellow_gold_price'] * 1.10).'&nbsp; </TD></TR>
			<TR>
			<TD width=509 height=22>&nbsp;Platinum Price:'.round($detail['platinum_price'] * 1.10).'</TD></TR>
			<TR>
...
<div>
    	  ...
    	  if($action == 'delete'){
					...
					if(is_array($post)){
						...
						foreach($post as $index => $value) {
							...
							if($index==$action.'btn') continue;

							if($action=='edit')  {
								if(in_array($value, $skipField)) continue;
							 }
			 		...
			 		if($action == 'add')
			 		{
				 		...
				 		$isinsert = $this->db->insert($this->config->item('table_perfix').'new_product',
				 				$field
							);
			 		   $rid = $this->db->insert_id() ;
			 		...
			 		$rid = $id;	
			 		$this->db->where('id' , $id);
			 		$isinsert = $this->db->update($this->config->item('table_perfix').'new_product',
			 				$field
						);
					//$productArr = $this->getAllByProductID($rid);
					  // $this->addProducttoEbay($productArr);
				...
				if($isinsert) $retuen['success'] .= '<h1 class="success">Product info Successfully '.ucfirst($action).'ed .</h1><br /><br /><small> <a href="'.config_item('base_url').'admin/product/edit/'.$post['categoryid'].'/'.$rid.'">Click Here </a> To View/Edit</small>';
         }			
 ...
 function getcategory( $page =1 , $rp = 10 ,$sortname = 'category_name' ,$sortorder = 'desc' ,$query= '' , $qtype = 'category_name' , $oid = ''){
		$results = array();
			...
			$qwhere = "";
			if ($query) $qwhere .= " AND $qtype LIKE '%$query%' ";
			if($oid != '') $qwhere .= " AND parentid = $oid";
	 ...
	 function getProduct( $page =1 , $rp = 10 ,$sortname = 'id' ,$sortorder = 'desc' ,$query= '' , $qtype = 'categoryid' , $oid = '', $section ){
		$results = array();
			...
			$qwhere = "";
			if ($query) $qwhere .= " AND $qtype = '$query' ";
			//if($oid != '') $qwhere .= " AND id = $oid";
    	  ...
    	  if($action == 'delete'){
					...
					if(is_array($post)){
			 		...
			 		if($action == 'add')
			 		{
				 		...
				 		$isinsert = $this->db->insert($this->config->item('table_perfix').'category',
				 				array(
			 		...
			 		if($action == 'edit')
			 		{
						...
						$this->db->where('id' , $id);
						$isinsert = $this->db->update($this->config->item('table_perfix').'category',
								array(
									  'category_label'		=> $category_label,
									  'category_description' => $category_description,
									  'parentid'		=> $parentid,
				...
				if($catArr['ebay_cat_id']=='' || $catArr['ebay_cat_id']==0) {
                                   $catArr['Action'] = 'Add';
                                   $this->_setStoreCategories($catArr);
                                    //$ebay_cat_id = $this->addCustomEbayCategory($catArr);
				} else {
                                    $catArr['Action']           = 'Rename';
                                    $catArr['SC_CC_CategoryID'] = $catArr['ebay_cat_id'];
                                    $catArr['SC_CC_Name']       = $catArr['category_label'];
                                    $this->_setStoreCategories($catArr);
                                     //$ebay_cat_id = $this->updateCustomEbayCategory($catArr);
				}
	          if($isinsert) $retuen['success'] .= '<h1 class="success">Category info Successfully '.ucfirst($action).'ed .</h1><br /><br /><small> <a href="'.config_item('base_url').'admin/category/edit/'.$parentid.'/'.$rid.'">Click Here </a> To View</small>';
         }			
  ...
  * date dec 8 2010
  * version 1
  * desc : Managing Store Categories
  * method name : _setStoreCategories();
  */

         function _setStoreCategories($params){
            ...
            try {
                ...
                $params['ErrorLanguage']    = $compatibilityLevel;
			...
			$dest_cat_id = '-999';
                        ...
                        $dest_cat_id = $params['SC_CC_CategoryID'];
                    }
                    //$params['DestinationParentCategoryID']  = $params['DestinationParentCategoryID'];
                    $params['ItemDestinationCategoryID']    = $dest_cat_id;
                }
                if(!empty($params['SC_CC_CategoryID'])){
                    $params['StoreCategories']['CustomCategory']['CategoryID']                      = $params['SC_CC_CategoryID'];
                    $params['StoreCategories']['CustomCategory']['Name']                            = $params['SC_CC_Name'];
                    if(!empty ($params['SC_CC_ChildCat_CategoryID'])){
                        $params['StoreCategories']['CustomCategory']['ChildCategory']['CategoryID']     = $params['SC_CC_ChildCat_CategoryID'];
                        $params['StoreCategories']['CustomCategory']['ChildCategory']['ChildCategory']  = $params['SC_CC_ChildCat_ChildCategory'];
                        $params['StoreCategories']['CustomCategory']['ChildCategory']['Name']           = $params['SC_CC_ChildCat_Name'];
                        $params['StoreCategories']['CustomCategory']['ChildCategory']['Order']          = $params['SC_CC_ChildCat_Order'];
                    }
                }

                 $results = $client->SetStoreCategories($params);
                 if($results->Ack == 'Success'){
                     $responseMsg = $results->Status;
                 }else{
                     $responseMsg = $results->Status;
                 }
   ...
   */
	...
	function addProducttoEbay($detail, $action='Add'){
		...
		$watchDetail .='<br><B>Brand:</B>'.ucfirst($detail['brand']);
		$watchDetail .='<br><B>Retail:</B>'.ucfirst($detail['price1']);
		$watchDetail .='<br><B>Our Price:</B>'.ucfirst($detail['price2']);
		$watchDetail .='<br><B>Gender:</B>'.ucfirst($detail['gender']);
		$watchDetail .='<br><B>Case:</B>'.$metalarray[$detail['metal']];
		$watchDetail .='<br><B>Band:</B>'.ucfirst($detail['band']);
		$watchDetail .='<br><B>Dial:</B>'.ucfirst($detail['dial']);
