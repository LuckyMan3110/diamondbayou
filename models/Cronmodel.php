	 ...
	 if(is_array($cols))
	  {
		...
		$Flour 		= isset($cols[13]) ? $cols[13] : '';
		$Meas 		= isset($cols[15]) ? $cols[15] : '0';
		$Comment 	= isset($cols[42]) ? $cols[42] : '';
		...
		$certimage  = isset($cols[51]) ? $cols[51] : '';
		$length 	= isset($cols[16]) ? $cols[16] : '';
		$width 		= isset($cols[17]) ? $cols[17] : '';
		$height 	= isset($cols[18]) ? $cols[18] : '';
		
		$lab 		= isset($cols[20]) ? $cols[20] : '';
		$fcolor_intens = isset($cols[8]) ? $cols[8] : '';
		$crown 		= isset($cols[39]) ? $cols[39] : '';
		$crown_angle = isset($cols[75]) ? $cols[75] : '';
		$pavilion 	= isset($cols[40]) ? $cols[40] : '';
		$paviil_angle = isset($cols[71]) ? $cols[71] : '';
       ...
       //     if (in_array($color , $os)) {
		    ...
		    $cerstr = substr($Cert,0,5);
              /*      if(($cerstr == 'GIA' || $cerstr == 'EGL I' || $cerstr == 'EGL U') && !empty($Cert_n) &&  ($price > 0 ) ){      */                  
					 ...
					 $rappnet_values = "('".cleans($lot)."', '".cleans($owner)."', '".cleans($shape)."', '".cleans($carat)."', '".cleans($color)."', '".cleans($fancy_color)."', '".cleans($fancy_color_ot)."', '".cleans($clarity)."', '".cleans($price)."', '".cleans($Rap)."', '".cleans($Cert)."', '".cleans($Depth)."', '".cleans($TablePercent)."', '".cleans($Girdle)."', '".cleans($Culet)."', '".cleans($Polish)."', '".cleans($Sym)."', '".cleans($Flour)."', '".cleans($Meas)."', '".cleans($Comment)."', '".cleans($Stones)."', '".cleans($Cert_n)."', '".cleans($Stock_n)."', '".cleans($Make)."', '".cleans($Date)."', '".cleans($City)."', '".cleans($State)."', '".cleans($Country)."', '".cleans($ratio)."', '".cleans($cut)."', '".cleans($tbl)."', '".cleans($pricepercrt)."', '".cleans($certimage)."', '".cleans($length)."', '".cleans($width)."', '".cleans($height)."', '".cleans($lab)."', '".cleans($fcolor_intens)."', '".cleans($crown)."', '".cleans($crown_angle)."', '".cleans($pavilion)."', '".cleans($paviil_angle)."')";
