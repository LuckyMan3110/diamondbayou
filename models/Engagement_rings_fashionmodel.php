...
*/
class Engagement_rings_fashionmodel extends CI_Model{
    ...
    function getStullerItemRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'name', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
    
        $results = array();

		$sort = "ORDER BY $sortname $sortorder";
		//$sort = "GROUP BY ur.productId";
		$start = (($page - 1) * $rp);
        ...
        if( !empty($category) AND !empty($sub_cate_id) ) {
            $qwhere .= " AND `catid` = '{$sub_cate_id}'";
        }else{
            if( !empty($category) ) {
                $qwhere .= " AND `catid` = '{$category}'";
            }  
        } 
        ...
        //$sub_cate_id
		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'us where 1=1 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		
		$priceMargin = 0;
		
		//$sql = 'SELECT *, ud.id ring_id, ur.priceRetail AS priceRetail FROM dev_us AS ud INNER JOIN dev_us_prices AS ur ON ud.`name` = ur.productId WHERE 1=1 AND ur.priceRetail >= 100 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		//var_dump($sql);
        //file_put_contents('jewelry_query', $sql);
                
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = 'SELECT id FROM  ' . $this->config->item('table_perfix') . 'us  where 1=1 ' . $qwhere;
