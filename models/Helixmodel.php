...
class Helixmodel extends CI_Model {
	...
	function saveinhelix($cols){
		if(is_array($cols)){
			...
			$isinsert = $this->db->insert($this->config->item('table_perfix').'helix_products',array(
				'lot'    =>  $lot,
				'owner' =>  $owner,
				'shape' =>  $shape,
				'carat' => $carat,
			...
			if(!$isinsert){
				$isinsert = $this->db->insert('error',array('msg'	=> $lot . '-helix_products insert faild'));
				return true;
			}else{
				return true;
			}
	...
	function getprices( $page =1 , $rp = 10 ,$sortname = 'pricefrom' ,$sortorder = 'asc' ,$query= '' , $qtype = 'pricefrom' , $oid = '', $unique=''){
		$results = array();
		...
		$table_name = ( !empty($unique) ? 'unique_prices' : 'helix_prices' );
		$sql = 'SELECT  * FROM  '. $this->config->item('table_perfix').$table_name.' where 1=1 '. $qwhere . ' ' . $sort . ' '. $limit;
	 	$result = $this->db->query($sql);
		$results['result']  = $result->result_array();	
		$sql2 = 'SELECT rowid FROM  '. $this->config->item('table_perfix').$table_name.'  where 1=1 '. $qwhere;
	...
	function getPriceByStullerID($id = 0){
		$qry = "SELECT * FROM ".config_item('table_perfix')."stuller_prices WHERE rowid = '".$id."'";
		$result = 	$this->db->query($qry);
	...
	function getPriceByQualityID($id = 0){
		$qry = "SELECT * FROM ".config_item('table_perfix')."quality_prices WHERE rowid = '".$id."'";
		$result = 	$this->db->query($qry);
		$return = $result->result_array();
	...
	function helixprices($post , $action = 'view' , $id =0, $isUnique=''){
		$retuen  = array();
		$retuen['error'] 	= '';
		$retuen['success'] 	= '';
		$tableName = ( !empty($isUnique) ? 'unique_prices' : 'helix_prices' );
		if($action == 'delete'){
			$items = rtrim($_POST['items'],",");
			$sql = "DELETE FROM ".$this->config->item('table_perfix')."$tableName WHERE rowid IN ($items)";
			$result = $this->db->query($sql);
			$total = count(explode(",",$items)); 
			$retuen['total'] = $total;
		}else{
			...
			if( ($pricefrom < $priceto ) && ($pricefrom >0) && ($priceto > 0)){
				...
				if($action == 'edit'){
			 		$this->db->where('rowid' , $id);
			 		$isinsert = $this->db->update($this->config->item('table_perfix').$tableName.'',
			 		array(			  
						'pricefrom' => $pricefrom,
						'priceto'	  => $priceto,
						'rate'	  => $rate
	...
	function getPriceByID($id = 0, $unique=''){
		$tableName = ( !empty($unique) ? 'unique_prices' : 'helix_prices' );
		$qry = "SELECT * FROM ".config_item('table_perfix')."$tableName WHERE rowid = '".$id."'";
		$result = 	$this->db->query($qry);
		$return = $result->result_array();
	...
	function erdprice($price = 250){
		...
		$qry = "SELECT rate from ".config_item('table_perfix')."helix_prices WHERE  pricefrom <= $price and priceto >= $price ";
		$result = 	$this->db->query($qry);
		$return = $result->result_array();	
		if(isset($return[0])){
	...
	function getallDaimonds($table_name){
		$this->db->select("uid,price_incsv,vdbapp_id");
