...
*/
class Jcartfashionmodel extends CI_Model{
    ...
    function getStullerItemRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
     /*   $results = array();
	    $qwhere = '';
	   ...
	   if($query){
		...
		mysql_query("RESET QUERY CACHE");
                    $result = mysql_query("SELECT * FROM `dev_jewelries` WHERE `title`='".$query."'");
        ...
        $sql = 'SELECT * FROM `dev_jewelries` where 1 = 1 ' . $qwhere . ' ' . $sort . ' ' . $limit;

        $result = $this->db->query($sql);
        $results['result'] = $result->result_array();
        $sql2 = 'SELECT `id` FROM `dev_jewelries` where 1 = 1 ' . $qwhere;
      ...
      $results = array();
        ...
        if( !empty($category) ) {
            $qwhere .= " AND `category` = '{$category}'";
        }  
        
        if( !empty($sub_cate_id) ) {
            $qwhere .= " AND `subcategory` = '{$sub_cate_id}'";
        }  
        $qwhere .= " AND `vendor_name` = 'Carrolls Jewelers'";
	...
	function getStullerItemRowsMarketplaceJewelry($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
        ...
        $qwhere .= " AND `vendor_name` != 'Gold Source'";
        $qwhere .= " AND `vendor_name` != 'Johnny Dang'";
        $qwhere .= " AND `vendor_name` != '713-777-2026'";
        $qwhere .= " AND `vendor_name` != 'Johnny Dang and Co'";
        $qwhere .= " AND `category` != '740520129'";
        $qwhere .= " AND `category` != '740520271'";
        $qwhere .= " AND `category` != '740520211'";
        $qwhere .= " AND `category` != '740520272'";
        $qwhere .= " AND `category` != '740520259'";
        $qwhere .= " AND `category` != '740520120'";
        $qwhere .= " AND `vendor_name` = 'Demo'";
	...
	function getStullerItemOvermountingRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
        ...
        if( !empty($category) ) {
			$sqlCat = "SELECT category_name FROM dev_ebaycategories WHERE category_id = '".$category."'";
            $sqlCat .= " ORDER BY category_name ASC";
            $resultCat = $this->db->query($sqlCat);	
            $categories = $resultCat->row();
            $qwhere .= " AND `category` LIKE '{$categories->category_name}'";
        }  
        if( !empty($sub_cate_id) ) {
			$sqlSubCat = "SELECT category_name FROM dev_ebaycategories WHERE category_id = '".$sub_cate_id."'";
            $sqlSubCat .= " ORDER BY category_name ASC";
            $resultSubCat = $this->db->query($sqlSubCat);	
            $subcat = $resultSubCat->row();
            $qwhere .= " AND (`subcategory` LIKE '". $subcat->category_name ."' OR `subcategory2` LIKE '". $subcat->category_name ."')";
        }  
        $qwhere .= " AND `vendor_name` LIKE 'Overnight Mountings'";
    ...
    function getStullerItemRowsMarketplace360_Engagement_Rings($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
	...
	function getQualityGoldItemRowsMarketplaceJewelry($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
		...
		$qwhere .= " AND (category =  '740520272'
		OR category =  '740520211'
		OR category =  '740520271'
		OR category =  '740520129'
		)";
		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'jewelries where 1=1 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
    ...
    function getStullerItemRowsOverMounting($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		...
		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'jewelries where 1=1 AND `category` = 740520120 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = 'SELECT stock_number FROM  ' . $this->config->item('table_perfix') . 'jewelries  where 1=1 AND `category` = 740520120 ' . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
    ...
    function getStullerItemDetailViaID($product_id=0) {
        $sql = "SELECT * FROM `dev_jewelries` WHERE stock_number = '{$product_id}'";
        $result = $this->db->query($sql);
	$results  = $result->result_array();
...
/// get colection item imgs
	...
	function getSubCategory($cateid='') {
        if($cateid != 0) {
			$sql = "SELECT * FROM dev_ebaycategories WHERE parent_id = '".$cateid."'";
	...
	function getCostarItemRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
		...
		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'costar_diamonds where 1=1 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = 'SELECT stock_number FROM  ' . $this->config->item('table_perfix') . 'costar_diamonds  where 1=1 ' . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
	...
	function getDiamondTraceItemRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
        ...
        if( !empty($filter_id) && $filter_id != 340) {
			if($filter_id == 990199){
				$qwhere .= " AND `category` LIKE 'Halo'";
			}elseif($filter_id == 990299){
				$qwhere .= " AND `category` LIKE 'Side Stone'";
			}elseif($filter_id == 990399){
				$qwhere .= " AND `category` LIKE 'Three Stone'";
			}elseif($filter_id == 990499){
				$qwhere .= " AND `category` LIKE 'Vintage'";
			}
        }  
		$qwhere .= " AND `costar_id` = 0 AND `overnight_id` = 0 AND `dev_us_id` = 0";
        if( !empty($sub_cate_id) ) {
			$sqlSubCat = "SELECT category_name FROM dev_ebaycategories WHERE category_id = '".$sub_cate_id."'";
            $sqlSubCat .= " ORDER BY category_name ASC";
            $resultSubCat = $this->db->query($sqlSubCat);	
            $subcat = $resultSubCat->row();
            $qwhere .= " AND (`sub_category` LIKE '". $sub_cate_id ."')";
        }

		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'engagement_rings where 1=1 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = 'SELECT item_number FROM  ' . $this->config->item('table_perfix') . 'engagement_rings  where 1=1 ' . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
	...
	function getRichardCannonItemRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
        ...
        if( !empty($filter_id) && $filter_id != 340) {
			if($filter_id == 990199){
				$qwhere .= " AND `category` LIKE 'Anklets'";
			}elseif($filter_id == 990299){
				$qwhere .= " AND `category` LIKE 'Bangles'";
			}elseif($filter_id == 990399){
				$qwhere .= " AND `category` LIKE 'Bracelets'";
			}elseif($filter_id == 990499){
				$qwhere .= " AND `category` LIKE 'Chains'";
			}elseif($filter_id == 990599){
				$qwhere .= " AND `category` LIKE 'Charms'";
			}elseif($filter_id == 990699){
				$qwhere .= " AND `category` LIKE 'Earrings'";
			}elseif($filter_id == 990799){
				$qwhere .= " AND `category` LIKE 'Necklaces'";
			}elseif($filter_id == 990899){
				$qwhere .= " AND `category` LIKE 'Pendants'";
			}elseif($filter_id == 990999){
				$qwhere .= " AND `category` LIKE 'Rings'";
			}elseif($filter_id == 991099){
				$qwhere .= " AND `category` LIKE 'Toe Rings'";
			}
        }
		$qwhere .= " AND is_carol = 0";
		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'jewelries_new where 1=1 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = 'SELECT stock_number FROM  ' . $this->config->item('table_perfix') . 'jewelries_new  where 1=1 ' . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
	...
	function getVdbappItemRows($viewType='', $filter_id=0, $category='', $page = 0, $rp = 8, $sortname = 'title', $sortorder = 'desc', $query = '', $qtype = 'id', $oid = '') {
		$results = array();
		$sort = "ORDER BY $sortname $sortorder";
		$start = (($page - 1) * $rp);
		$limit = "LIMIT $start, $rp";
        ...
        if( !empty($sub_cate_id) ) {
            ...
            $resultSubCat = $this->db->query($sqlSubCat);	
            $subcat = $resultSubCat->row();
            $qwhere .= " AND `subcategory` LIKE '". $subcat->category_name ."'";
        }

		$sql = 'SELECT * FROM ' . $this->config->item('table_perfix') . 'vdbapp_jewelry where 1=1 ' . $qwhere . ' ' . $sort . ' ' . $limit; 
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = 'SELECT stock_number FROM  ' . $this->config->item('table_perfix') . 'vdbapp_jewelry  where 1=1 ' . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
