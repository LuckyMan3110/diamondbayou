...
class Jewelleriesmodel extends CI_Model{
    ...
    //// get unique table
    function getUniqueTable($cid=0) {
        $solitair_cateid = solitairesCateID();  /// Engagement rings solitaires categories id list
        if( in_array($cid, $solitair_cateid) ) {
            $tableName = '`dev_us`';
        } else {
            $tableName = '`dev_us_finish_stones`';
        }
        return $tableName;
    }

    function numOfRowsofUnique($catid){
        $table_name = $this->getUniqueTable($catid);
        $sql = 'SELECT  COUNT(*) as records FROM '.$table_name.' WHERE catid='.$catid.'';
    ...
    function getUniqueFromScrap($per_pg,$offset,$catname,$carat=''){
        $table_name = $this->getUniqueTable($catname);
        if($carat=='') { $caratsql='';}
        else{
        ...
        $sql = "SELECT  * FROM $table_name WHERE id <> '' AND image <> '' AND catid=".$catname.''.$caratsql.$limit;
        $result = $this->db->query($sql);
        $results['result'] = $result->result_array();
        //echo $sql;exit;
        return $results;
    ...
    function getEngagementQualityFromScrap($per_pg, $offset,$catname,$carat='',$sort='ASC'){
        ...
        $limit=" LIMIT ".$offset.",".$per_pg;
        $sql = 'SELECT * FROM '.$table_name.' WHERE name like "%'.str_replace('_', ' ', $catname).'%"'.$caratsql.$limit;
        $result = $this->db->query($sql);
        $results['result'] = $result->result_array();
        return $results;
    }
	...
	function buildChildFromScrap($oldID){
...
//	$price=" AND price_website='".$price."'";
	...
	public function addtoshoppingcartmode($post, $type='R'){
		...
		$finish_level = (isset($post['finish_level']) && $post['finish_level'] !='' ? $post['finish_level'] : 'NA' );
		$itm_metal = (isset($post['txt_metal']) && $post['txt_metal'] !='' ? $post['txt_metal'] : 'NA' );
		...
		$stock_number   = (isset($post['stock_number']) && $post['stock_number'] !='' ? $post['stock_number'] : $post['prid'] );
		$vendor_number  = (isset($post['vendor_number']) && $post['vendor_number'] !='' ? $post['vendor_number'] : '' );
		$table_name     = (isset($post['table_name']) && $post['table_name'] !='' ? $post['table_name'] : '' );
		$item_title     = (isset($post['item_title']) && $post['item_title'] !='' ? $post['item_title'] : '' );
		$item_url       = (isset($post['item_url']) && $post['item_url'] !='' ? $post['item_url'] : '' );
		$product_type   = (isset($post['product_type']) && $post['product_type'] !='' ? $post['product_type'] : '' );
		$item_type  = (isset($post['item_type']) && $post['item_type'] !='' ? $post['item_type'] : 'NA' );
		$diamond_cert_no  = (isset($post['diamond_cert_no']) && $post['diamond_cert_no'] !='' ? $post['diamond_cert_no'] : 'NA' );
		$diamond_id = (isset($post['diamond_id']) && $post['diamond_id'] !='' ? $post['diamond_id'] : '' );
		$uniqe_link = $this->session->userdata('unique_ringid');
		$uniqe_link = ( !empty($uniqe_link) ? $uniqe_link : '' );
		$netOrignalPrice = (isset($post['price']) && $post['price'] !='' ? $post['price'] : '' );
		$unique_pagelink = (isset($post['unique_pagelink']) && $post['unique_pagelink'] !='' ? $post['unique_pagelink'] : '' );
		$uniquepage_link = ( !empty($unique_pagelink) ? $unique_pagelink : $uniqe_link );
		$ez_payments = (isset($post['ez_payments']) && $post['ez_payments'] !='' ? $post['ez_payments'] : '' );
        $ezpay = ( $ez_payments > 0 ? ( $price / $ez_payments ) : 0 );
		$cart_adoption = ( ( isset($post['txt_addoption']) && !empty($post['txt_addoption']) ) ? $post['txt_addoption'] : 'addunique' );
		$vendors_name = ( ( isset($post['vendors_name']) && !empty($post['vendors_name']) ) ? str_replace("'","\'",$post['vendors_name']) : 'Unique' );
		$qty = (isset($post['txt_qty']) && $post['txt_qty'] != '')? $post['txt_qty'] : 1;
		$cartWirePrice = $price * $qty;
		$prodname = !empty($post['prodname'])?$post['prodname']:$item_title;
		$txt_engtext = ((isset($post['txt_engtext']) && !empty($post['txt_engtext'])) ? $post['txt_engtext'] : '');
		$font_style = ((isset($post['font_style']) && !empty($post['font_style'])) ? $post['font_style'] : '');

		$sql = "INSERT INTO " . $this->config->item('table_perfix') . "cart (sessionid, lot, stock_number, diamond_id, sidestone1, ezpay, venderinfo, vendorname, vendor_number, table_name, item_title, item_url, product_type,  price, quantity, addoption, totalprice, dsize, `name`, image_url, item_metal, item_type, `unique_urlink`, `netorder_price`, `engraved_text`, `engraved_font`, `order_type`, `default_ringsize`, `finish_level`, `metal_weight`, `ezpay_option`, `diamond_cert_no`)  
		VALUES ('$sessionid', '".$post['prid']."', '".$stock_number."', '{$diamond_id}', '{$center_stone_id}' ,'$ezpay', '{$post['vender']}', '{$vendors_name}', '{$vendor_number}', '{$table_name}', '{$item_title}', '{$item_url}', '{$product_type}', '$price', '$qty','$cart_adoption', '$cartWirePrice','$itemring_size', '{$prodname}', '{$post['url']}', '$itm_metal', '$item_type', '$uniquepage_link', '$netOrignalPrice', '{$txt_engtext}', '{$font_style}', '$type', '{$finger_size}', '{$finish_level}', '{$metals_weight}', '{$ez_payments}', '{$diamond_cert_no}')";
	...
	function getringsdetails($bdname,$dbid,$id){
		$sql = "SELECT * FROM ".$bdname." where ".$dbid." = '".$id."'";
		$result = $this->db->query($sql);
		$results = $result->result_array();
	...
	function getbanddetails($id){
		$sql = "SELECT *, ud.id as ring_id, ur.priceRetail FROM dev_us AS ud INNER JOIN dev_us_prices AS ur ON ud.`name` = ur.productId WHERE ud.id = '".$id."'";
		$result = $this->db->query($sql);
		$results = $result->result_array();
	...
	function getdiamonddetails($id){
		$sql = "SELECT * FROM dev_index WHERE lot LIKE '".$id."'";
		$result = $this->db->query($sql);
		$results = $result->result_array();
