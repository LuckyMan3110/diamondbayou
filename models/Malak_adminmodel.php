...
class malak_adminmodel extends CI_Model {

	function __construct() {
		parent::__construct();
		//$malak_db = $this->load->database('malaknew', TRUE); // Connect to malak database
		//$this->db = $malak_db;
	}
	...
	function saveCommonPageTemplate($pageid = '', $content = '', $title = '') {
		...
		$t = $this->db->update($this->config->item('table_perfix') . 'companyinfo', array('content' => $content, 'description' => $title));
		if ($t) {
			return true;
		} else {
			return false;
        ...
        ////// import jewelry data from xl into database
	function import_xlsfile_data($arrayData) {
		...
		//$isinsert = $this->db->insert($this->config->item('table_perfix') . 'jewelries', $arrayData);
		$tt_record = count($arrayData);
		
		if($tt_record > 1) {
			for($i=0; $i<$tt_record; $i++) {
				$insert_data = $this->db->insert($this->config->item('table_perfix') . 'jewelries', $arrayData[$i]);
			}
		} else {
			$insert_data = $this->db->insert($this->config->item('table_perfix') . 'jewelries', $arrayData[0]);	
		}
		
		//$insert_data = $this->db->insert_batch($this->config->item('table_perfix') . 'jewelries', $arrayData);
		
						//$rid = $this->db->insert_id();
						//$isinsert = $this->db->insert($this->config->item('table_perfix') . 'gemstones', array('jew_id' => $rid));
		
		//$retuen['qr'] = $this->db->last_query();
		if($insert_data) {
		$retuen['success'] = '<h1 class="success">Ring info Successfully Added .</h1><br /><br /><small> <a href="' . config_item('base_url') . 'admin/jewelries_manager/add/">Click Here </a> To View/Edit</small>';
		} else {
			$retuen['error'] = 'Error : Data has not inserted successfully!';
		}
		
		return $retuen;
	}
        
    function update_jew_collection($id=0, $itemsku='', $image_url='') {
        $image_link = str_replace("_slash_", "/", $image_url);
        
        if( !empty($itemsku) || !empty($image_url) ) {
            $sql = "UPDATE dev_jewelries SET ";
            $sql .= " item_sku = '{$itemsku}'";
            $sql .= " , jew_image_url = '{$image_link}'";

            $sql .= " WHERE stock_number = '{$id}'";
	...
	function jewelries($post, $action = 'view', $id = 0, $global_fields='') {
		...
		if ($action == 'delete') {
			...
			if (is_array($post)) {
				...
				if ($_FILES['image1']['name'] != '') {
				    $this->uploadfile($_FILES, 'image1', 'uploads', 'jpeg,gif,jpg,bmp,png', 'images/uploads', $rid, 'jewelries', 'stock_number', $rid, 'image1');
                }

				if ($_FILES['image2']['name'] != '')
				$this->uploadfile($_FILES, 'image2', 'uploads', 'jpeg,gif,jpg,bmp,png', 'images/uploads', $rid, 'jewelries', 'stock_number', $rid, 'image2');


				if ($_FILES['image3']['name'] != '')
				$this->uploadfile($_FILES, 'image3', 'uploads', 'jpeg,gif,jpg,bmp,png', 'images/uploads', $rid, 'jewelries', 'stock_number', $rid, 'image3');

				if ($_FILES['image4']['name'] != '')
				$this->uploadfile($_FILES, 'image4', 'uploads', 'jpeg,gif,jpg,bmp,png', 'images/uploads', $rid, 'jewelries', 'stock_number', $rid, 'image4');

				if ($_FILES['image5']['name'] != '')
				$this->uploadfile($_FILES, 'image5', 'uploads', 'jpeg,gif,jpg,bmp,png', 'images/uploads', $rid, 'jewelries', 'stock_number', $rid, 'image5');

				if ($_FILES['image6']['name'] != '')
				$this->uploadfile($_FILES, 'image6', 'uploads', 'jpeg,gif,jpg,bmp,png', 'images/uploads', $rid, 'jewelries', 'stock_number', $rid, 'image6');


				$productArr = $this->getAllByStockID($rid);
				$ebayCatQuery = "SELECT  parentid ,  ebay_cat_id  FROM  " . $this->config->item('table_perfix') . "category where id = '".$productArr['category']."'";
				$result = $this->db->query($ebayCatQuery);
				$data = $result->result_array();
				$productArr['store_cat_id'] = $data[0]['ebay_cat_id'];
				$productArr['parentid'] = $data[0]['parentid'];
		...
		if ($isinsert)
		$retuen['success'] .= '<h1 class="success">Ring info Successfully ' . ucfirst($action) . 'ed .</h1><br /><br /><small> <a href="' . config_item('base_url') . 'admin/jewelries_manager/edit/' . $rid . '">Click Here </a> To View/Edit</small>';
		return $retuen;
	}
	...
	function uploadfile($files, $formfilenvar = 'myfile', $imageurlbase = '', $extsupport = 'jpeg,gif,jpg,bmp,png,swf', $base_savepath = 'uploads/', $filename = 'error.error', $dbtable = 'error', $idfield = 'id', $idvalue = '1', $tablefield = 'msg') {
		...
		//echo $formfilenvar; exit();
		
		if ($files[$formfilenvar]['name'] != '') {
		    
			$supportExt = explode(',', $extsupport);
			$fileExt = explode('.', $files[$formfilenvar]['name']);
                        $fileExtenion = end($fileExt);
                        
			if (in_array(strtolower($fileExtenion), $supportExt)) {
				...
				//	echo '<pre>'; print_r($imageurl);  exit();
				...
				if ($ismove) {
				   ...
				   //echo $imageurl; echo '<br>';
				    //echo $this->config->item('table_perfix');
				   //exit();
	...
	function sendRequestEbay($requestArray, $section = 'watches') {
		...
		if ($ack == 'Failure') { //echo '<br>'.die('xyz');
			...
			if ($section == 'pendants') {
				...
				$isinsert = $this->db->update($this->config->item('table_perfix') . 'loosediamonds', array(
                    'ebayid' => $itemID,
				));
...
document.getElementById(\'tab_5\').className = \'\';
	...
	function getCategoryNameByID($id) {
		$sql = "select category_name from " . $this->config->item('table_perfix') . "ebaycategories where category_id = '" . $id . "'";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}
	...
	function getRapnetStones($page = 1, $rp = 10, $sortname = 'lot', $sortorder = 'desc', $query = '', $qtype = 'lot', $oid = '') {
		$results = array();
		...
		$sql = "SELECT  *  , (price * carat) as re  FROM  " . $this->config->item('table_perfix') . "rapproduct where  Cert IN ('GIA','EGL U','EGL I') " . $qwhere . " " . $sort . " " . $limit;
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = "SELECT lot FROM  " . $this->config->item('table_perfix') . "rapproduct where 1=1 " . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
		return $results;
	}
	...
	function getAllowners() {
		$sql = "SELECT  * FROM  " . $this->config->item('table_perfix') . "owners where 1=1 ";
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		return $results;
	}
			...
			if (is_array($post)) {
				if ($action == 'add') {
					...
					if ($_FILES['picture1']['name'] != '') {
						$this->uploadfile($_FILES, 'picture1', 'images/pictures', 'jpeg,gif,jpg,bmp,png', config_item('base_path') . 'images/pictures', $post['diamondshape'], 'pictures', 'diamondshape', $post['diamondshape'], 'picture1');
					}
		...
		if ($action == 'delete') {
			...
			foreach ($itemsArr AS $index => $value) {
				if ($value != '') {
					...
					$sql = "DELETE FROM dev_loosediamonds WHERE lot = $value";
					$result = $this->db->query($sql);
					if ($itemDetail[0]['ebayid'] != '-1' && $itemDetail[0]['ebayid'] != '-2') {
						$status = $this->endFixedPriceBid($itemDetail[0]['ebayid']);
					}
			...
			if (is_array($post)) {
				$action = '';
				if ($action == 'add') {
					$sql = "update dev_loosediamonds  set  category = '" . $_POST['category'] . "' WHERE lot = $id";
					$result = $this->db->query($sql);
	...
	function getPendantById($id) {
		$sql = "SELECT * FROM  dev_loosediamonds  WHERE lot = '" . $id . "'";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}

	function getPendants($page = 1, $rp = 10, $sortname = 'lot', $sortorder = 'desc', $query = '', $qtype = 'lot', $oid = '') {
		/* $results = array();
		...
		$qwhere = " ";
		$sort = " ORDER BY re ASC";
		$sql = "SELECT  * , (price * carat) as re FROM  dev_loosediamonds  where  Cert IN ('GIA','EGL U','EGL I') " . $qwhere . " " . $sort . " " . $limit;
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = "SELECT lot FROM  dev_loosediamonds  where 1=1 " . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
		return $results;
		*/
		...
		$sql = "SELECT  *  , (price * carat) as re  FROM  " . $this->config->item('table_perfix') . "loosediamonds where  Cert IN ('GIA','EGL U','EGL I') " . $qwhere . " " . $sort . " " . $limit;
		$result = $this->db->query($sql);
		$results['result'] = $result->result_array();
		$sql2 = "SELECT lot FROM  " . $this->config->item('table_perfix') . "loosediamonds where 1=1 " . $qwhere;
		$result2 = $this->db->query($sql2);
		$results['count'] = $result2->num_rows();
		return $results;
