<?php  
    class Sendjcartitemmodel extends CI_Model {
        ...
        function __construct()
        {
            ...
            //$this->load->helper('jcartfashionmodel');
    ...
    function sendJewItemToEbay($itemID=0) {
        ...
        if (isset($GetResponse) && !empty($GetResponse)) {
            $count++;
            $response .= "<div style=\"border: 1px solid red;display:block;\">Stock No($itemID)</br>";
            $response .= $GetResponse."</div>";
        } else {
            $response .="<div style=\"border: 1px solid red;display:block;\">Item Failed to Update on eBay Stock No($itemID)</div>";
        }         
    ...
    function addItemJewelryToEbay($item_id=0, $durations=30, $type, $index, $ebayRequest='AddItem') {
        ...
        $item_details = $this->getStullerItemDetailViaID($item_id);
        ...
        $item_price = $item_details['price_website'];
        $diamondSalePrice = $item_price;
        $salesPrice = $item_price;
        $daimondSalePrice = round($salesPrice,2);
        //$retail_price = $item_price * PRICE_PERCENT;
        $results_img = SITE_URL . 'images/' . addslashes($item_details['image1']); //$this->itemjewelrymodel->get_item_imgs( $item_details['id'] );   /// jewelry img list
...
<td><div class="image-galleryxxx">
 ...
 /* if( count($results_img) > 0 ) {
  ...
  $j = 1;
  $watchdetails .= '<img id="image'.$j.'" src="' . SITE_URL . 'images/' . addslashes($item_details['image1']) . '" width="550" height="550">';
  $imgthumb_list .='<li class="format"><a href="#image'.$j.'"><img src="' . SITE_URL . 'images/' . addslashes($item_details['image1']) . '" width="150" height="150"></a></li>';
...
<td colspan="3" align="center" class="headingx-2">Product Specifications</td>
        ...
        $watchdetails .= $item_details['item_title'].' '.$item_details['description'].' '.$item_details['ring_size'];
...
<!-- tabs  -->  
   ...
   // print_r($watchdetails); exit;
        ...
        $requestArray['startPrice'] = round($item_details['price_website']); //round($salesPrice);
        $requestArray['buyItNowPrice'] = round($item_details['price_website']); //round($salesPrice);
        $requestArray['quantity'] = '1';
        $requestArray['storeCategoryID'] = $storeCategoryId;
        $requestArray['lot'] = $item_details['stock_number'];
        $requestArray['itemID'] = $item_details['stock_number'];
        $requestArray['itemTitle'] = $item_details['item_title'];
        $requestArray['replace_gurantee'] = 'Days_14';
        $requestArray['ring_id'] = $item_details['stock_number'];
        $requestArray['category_id'] = $item_details['category'];
        //$requestArray['image1'] = config_item('base_url') . $diamondpic;
        $requestArray['image1'] = SITE_URL . 'images/' . addslashes($item_details['image1']);
        $requestArray['image2'] = SITE_URL . 'images/' . addslashes($item_details['image1']);
