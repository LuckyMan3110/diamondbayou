<?php  
    class Sendjcartwitemmodel extends CI_Model {
    ...
    function addItemJewelryToEbay($item_id=0, $durations=30, $type, $index, $ebayRequest='AddItem') {
        ...
        $item_price = $item_details['price1'];
        $diamondSalePrice = $item_price;
        $salesPrice = $item_price;
        $daimondSalePrice = round($salesPrice,2);
        //$retail_price = $item_price * PRICE_PERCENT;
        $results_img = SITE_URL . 'images/' . addslashes($item_details['thumb']); //$this->itemjewelrymodel->get_item_imgs( $item_details['id'] );   /// jewelry img list
...
<td><div class="image-galleryxxx">
 ...
 /* if( count($results_img) > 0 ) {
  ...
  $j = 1;
  $watchdetails .= '<img id="image'.$j.'" src="' . SITE_URL . '' . addslashes($item_details['thumb']) . '" width="550" height="550">';
  $imgthumb_list .='<li class="format"><a href="#image'.$j.'"><img src="' . SITE_URL . '' . addslashes($item_details['thumb']) . '" width="150" height="150"></a></li>';
...
<td colspan="3" align="center" class="headingx-2">Product Specifications</td>
        ...
        $watchdetails .= $item_details['productName'].' '.$item_details['productDescription'].' '.$item_details['movement'].' '.$item_details['band_length'];
...
<!-- tabs  -->  
   ...
   // print_r($watchdetails); exit;
        ...
        $requestArray['startPrice'] = round($item_details['price1']); //round($salesPrice);
        $requestArray['buyItNowPrice'] = round($item_details['price1']); //round($salesPrice);
        $requestArray['quantity'] = '1';
        $requestArray['storeCategoryID'] = $storeCategoryId;
        $requestArray['lot'] = $item_details['productID'];
        $requestArray['itemID'] = $item_details['productID'];
        $requestArray['itemTitle'] = $item_details['productName'];
        $requestArray['replace_gurantee'] = 'Days_14';
        $requestArray['ring_id'] = $item_details['productID'];
        $requestArray['category_id'] = $item_details['brand'];
        //$requestArray['image1'] = config_item('base_url') . $diamondpic;
        $requestArray['image1'] = SITE_URL . '' . addslashes($item_details['thumb']);
        $requestArray['image2'] = SITE_URL . '' . addslashes($item_details['thumb']);
