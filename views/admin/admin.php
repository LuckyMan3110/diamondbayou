...
function ideximport() {
    ...
    if($action == 'edit') {
        ...
        $data['id'] = $id;
                  ...
                  {display: 'Image URL', name : 'jew_image_url', width : 100, sortable : true, align: 'center'},
...
function websites_theme() {
    ...
    function jcart_download_excel($cateID, $gender_id, $subCateID) {
        ...
        $vendors_name = 'David Stern'; //$this->session->userdata('vendors_name');
        ...
        //if( $r < $stones_count && !empty($stones_count) ) {
        if( !empty($stones_count) ) {
        ...
        if( count($results) > 0 ) {
            foreach( $results as $rows ) {
                $exfile_rows .= '0' . TB;  /// attribte columns values
            }
        }
    ...
    function update_jewelry_cols($jewelry_id=0, $item_sku='', $imageLink='') {
        ...
        if( !in_array(strtolower(strrchr($image_link, '.')), $allowed_ext) ) {
            ...
            echo 'Item Sku and Image URL is updated Successfully! of Stock Number: ' . $jewelry_id;
        }       
	...
	function getjewelries_manager($addoption = '') {
		...
		foreach ($results['result'] as $row) {
			...
			$json .= ",'<input type=\"text\" size=\"10\" name=\"image_url\" onkeydown=\"update_jewelry_cols(\'".addslashes($row['stock_number'])."\')\" id=\"image_url_".addslashes($row['stock_number'])."\" onkeyup=\"update_jewelry_cols(\'".addslashes($row['stock_number'])."\')\" onblur=\"update_jewelry_cols(\'".addslashes($row['stock_number'])."\')\" onfocus=\"update_jewelry_cols(\'".addslashes($row['stock_number'])."\')\" id=\"item_sku_".addslashes($row['stock_number'])."\" value=\"".addslashes($image_file_name)."\">'";
