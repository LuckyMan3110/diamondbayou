...
<div>
							...
							<table width="80%" border="0" align="center" cellpadding="5" cellspacing="0" class="table_border">
								...
								<tr>
									...
									<td width="3%" align="left" valign="top"><dt id="edit-label">&nbsp;</dt><dd id="edit-element">
										<input type="submit"  name="<?=$action;?>btn" value="Continue to Send"  id="edit"  class="search_but"></dd>&nbsp;
									</td>          
	...
	<?php }else{ ?>
		...
		(function($){
			$(document).ready(function(){
				...
				$("a.send_batch_ebay").click(function(e){
					...
					$.post(
						'<?php echo SITE_URL . 'send_itemjewelry_to_ebay/send_bulk_item_jewelry_toebay' ?>',
						post_,
						function(data){
							if(data){
		...
		function update_jewitem_value(item_id) {
			...
			var url_link = base_url + 'hdering_admin_listings/update_jewelry_item/'+item_id+'/?embed_value='+embedLink;
		...
		function update_jcarat_value(item_id) {
			...
			var url_link = base_url + 'hdering_admin_listings/update_jewcarat_value/'+item_id+'/?center_carat='+center_carat+'&side_carat='+side_carat;
		...
		function update_new_price(item_id) {
			...
			var url_link = base_url + 'hdering_admin_listings/update_item_newprice/'+item_id+'/'+itemPrice;
		...
		function show_change_block(item_id) {
			var url_link = base_url + 'hdering_admin_listings/get_itemlist_popup/'+item_id;
			$.ajax({
				type:"POST",
				url:url_link,
				success:function(response) {
		...
		function change_item_diamondinfo(item_id, diamondID, rowsType) {
			var url_link = base_url + 'hdering_admin_listings/changeItemDiamondInfo/'+item_id+'/'+diamondID;
			...
			$.ajax({
				...
				success:function(response) {
					...
					window.location = base_url + 'hdering_admin_listings/heart_collection_items/view/' + view_type + '/' + filterID + '/' + item_category;
				},
				error:function(){alert('Error ');}
			});
		}
		...
		function findCenterDiamondList(item_id, item_price) {
			...
			var url_link = base_url + 'hdering_admin_listings/getCenterDiamondViaCaratSearch/'+item_id+'/'+item_shape+'/'+item_price+'/'+item_carat;
		...
		function findCenterDiamondListViaPrice(item_id, item_price, search_type) {
			...
			var url_link = base_url + 'hdering_admin_listings/getCenterDiamondViaCaratSearch/' + search_string; //alert(url_link);
		...
		function update_jitem_price(item_id) {
			...
			var url_link = base_url + 'hdering_admin_listings/update_jitems_price/'+item_id+'/'+item_price;
		...
		function jewelryimg(id) {
			$('#jewelryimg_'+id).modal('toggle');
		}

		function videomodel(id) {
			$('#videomodel_'+id).modal('toggle');
		}

		function changeDiamond(id) {
			$('#mymodel_'+id).modal('toggle');
		}
		</script> 
		...
		<form method="post" action="" enctype="multipart/form-data">
			...
			<input type="hidden" name="home_link" value="<?= $homeLink ?>">
		</form>
