				...
				<?php }else{ ?>
					...
					function set_option_sub_category_lists(){
						...
						$.ajax({
							type: 'post',
							dataType : 'html',
							url: '<?php echo base_url(); ?>jcart_fashion_jewelry/get_main_vdbapp_sub_category_list/?get_main_category_value='+get_main_category_value+'&get_filter_id_no='+get_filter_id_no,
					...
					<div class="set_cate_options">
						...
						<select onchange="set_option_sub_category_lists(this.value)" id="get_main_category_value">
							...
							<?php
							$where 	= array('parent_id' => '740520418');
							$categories	= $this->Catemodel->getdata_any_table_where($where, 'dev_ebaycategories');
							if(!empty($categories)){
								foreach($categories as $cats){
								?>
									<option value="<?= $cats->category_id ?>" <?php if($cats->category_id == $category_id){ echo 'selected'; } ?> >&nbsp;|-<?= $cats->category_name ?></option>
								<?php 
								}
							}
							?>
