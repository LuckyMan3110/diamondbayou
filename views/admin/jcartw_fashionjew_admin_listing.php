			...
			<div class="set_cate_options">
				...
				<select onchange="set_option_lists(this.value)">
					...
					foreach($brandoptions as $brands):
						$where_brand_name		= array('brand' => $brands['brand_name']);
						$get_count_brand_name	= $this->adminmodel->count_any_table($where_brand_name, 'dev_watches');
						if($get_count_brand_name > 0){        
							if($brands['brand_name'] == $category_id){
							?>
								<option value='<?= SITE_URL() ?>jcartw_fashion_jewelry/stuller_fashionjew_listing/view/jewelercart_watches/<?php echo $filter_id_no; ?>/<?php echo $brands['brand_name'] ?>' selected='selected'><?php echo $brands['brand_name'] ?></option>
							<?php }else { ?>
								<option value='<?= SITE_URL() ?>jcartw_fashion_jewelry/stuller_fashionjew_listing/view/jewelercart_watches/<?php echo $filter_id_no; ?>/<?php echo $brands['brand_name'] ?>'><?php echo $brands['brand_name'] ?></option>
							<?php
							}
						}
					endforeach;
