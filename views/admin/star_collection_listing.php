		...
		<div>
			<div>
				...
				(function($){
					$(document).ready(function(){
						...
						$("a.send_batch_ebay").click(function(e){
							...
							$.post('<?php echo SITE_URL . 'send_stuller_fashionjew_item_toebay/send_bulk_item_jewelry_toebay' ?>',post_,
							function(data){
								if(data){
						...
						$("a.delete_batch_ebay").click(function(e){
							...
							$.post('<?php print site_url("admin/deleteRapdiamondToEbay"); ?>', post_,
							function(data){
								if(data){
				...
				function findCenterDiamondListViaPrice(item_id, item_price, search_type) {
					...
					var url_link = base_url + 'admin_listings/getCenterDiamondViaCaratSearch/' + search_string;
				...
				<p class="selectbkbg">
					<input type="checkbox" value="1" id="chkb_etsy" onclick="selectedProd('chkb_etsy')">
					<label for="chkb_etsy">Send All Selected to Etsy </label> 
					<input type="hidden" value="" name="lotsnumber" id="lotsnumber">
				</p>
				<script>
				function set_option_sub_category_lists(){
					...
					$("#cat_id").val(get_main_category_value); 
					$.ajax({
						type: 'post',
						dataType : 'html',
						url: '<?php echo base_url(); ?>star_collection_jewelry/get_main_sub_category_list/?get_main_category_value='+get_main_category_value+'&get_filter_id_no='+get_filter_id_no,
				...
				function set_option_ringsMetal_lists(geVall){
					...
					$.ajax({
						type: 'post',
						dataType : 'html',
						url: '<?php echo base_url(); ?>star_collection_jewelry/get_prive_retail_by_metal_wise/?product_name='+product_name+'&ringsMetal='+ringsMetal,
						success: function (data) {
				...
				function updateJitemPriceMW(geVall, pName){
					...
					$.ajax({
						type: 'post',
						dataType : 'html',
						url: '<?php echo base_url(); ?>star_collection_jewelry/update_prive_retail_by_metal_wise/?product_name='+product_name+'&ringsMetal='+ringsMetal+'&ringsMetalPrice='+ringsMetalPrice,
						success: function (data) {
						}
					});
				}
				...
				<div class="set_cate_options">
					...
					<select onchange="set_option_lists(this.value)" id="show_sub_category_value">
						...
						<option value="<?= SITE_URL .'star_collection_jewelry/star_collection_listing/view/star_collection/'.$filter_id_no.'/'.$category_id.''; ?>"> - Show With Main Category</option> 
						<?php
						$where_main_cat	= $this->star_collection_fashionmodel->getSubCategory($category_id);
						if(!empty($where_main_cat)){
							foreach($where_main_cat as $main_cat){
							?>
								<option value="<?= SITE_URL .'star_collection_jewelry/star_collection_listing/view/star_collection/'.$filter_id_no.'/'.$category_id.'/'.$main_cat['id'].''; ?>" <?php if($main_cat['id'] == $item_sub_category){ echo 'selected'; } ?>> <?= $main_cat['name']; ?> </option>
							<?php 
							}
						}
						?>
					...
					<form action="<?= SITE_URL .'stuller_fashion_jewelry/downloadcsvbycategory'; ?>" method="GET" enctype="multipart/form-data">
						<input type="hidden" name="maincat_id" id="maincat_id" value="1">
						<input type="hidden" name="cat_id" id="cat_id" value="<?= $category_id ?>">
						<input type="hidden" name="subcat_id" id="subcat_id" value="<?= $item_sub_category ?>">
