    ...
    <section class="main_content_bg">
        <div class="container set_col_pading">
            <div class="col-sm-4 result_page_left">
                ...
                <?php 
                $incree = 1;
                $make_dynamic_main_fields	=  $this->diamondmodel->getdata_any_table_distinct_order_where('mall_elady_products', array('product_category'=>'Bags'), 'product_category', 'id');
                ...
                <div class="filter_block_section" style="display:;">
                    <div class="inner_sub_heading">
                        <?php 
                            echo $metal_name = $main_fields['product_category'];
                        ?>
                        <div class="clear"></div>
                    </div>
                    <?php 
                     $make_dynamic_fields1	=  $this->diamondmodel->getdata_any_table_distinct_order_where('mall_elady_products', array('product_category'=>$metal_name), 'product_sub_category', 'id');
                    foreach($make_dynamic_fields1 as $fields){
                    ...
                    ?>
						<div class="fiter_item_row">
							<label>
							<span><input name="clarity" type="checkbox" id="<?= $ch_id1 ?>" class="<?= $ch_id1 ?>" value="<?= $fields['product_sub_category'] ?>" /></span>
							<span><b><?= $fields['product_sub_category'] ?></b> (<?php echo getCountResult('mall_elady_products', array('product_sub_category' => $fields['product_sub_category']) ); ?>)</span>
							</label>
						</div>
                    <?php }
                    }
...
$( document ).ready(function() {
    ...
    var dataTable = $('#diamond_grid').DataTable({
        ...
        "ajax":{
          url :"<?php echo SITE_URL; ?>diamonds/bag_diamond_realtime/",
          type: "post",
          'data' : function(data){
            ...
            $incree = 1;
            $make_dynamic_main_fields	=  $this->diamondmodel->getdata_any_table_distinct_order_where('mall_elady_products', array('product_category' => 'Bags'), 'product_category', 'id');
            foreach($make_dynamic_main_fields as $fields1){
                 $make_dynamic_fields	=  $this->diamondmodel->getdata_any_table_distinct_order_where('mall_elady_products', array('product_category' => 'Bags'), 'product_sub_category', 'id');
                 foreach($make_dynamic_fields as $fields){
                    if($fields['product_sub_category']){
                ?>
                var M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?>         = '';
                if ($('#M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?>').is(":checked")){ M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?> = $('#M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?>').val(); }
                <?php 
                    }
                 }
            $incree++;
            ...
            foreach($make_dynamic_main_fields as $fields1){
                 ...
                 foreach($make_dynamic_fields as $fields){
                    if($fields['product_sub_category']){
                ?>
                data.M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?>        = M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?>;
                <?php 
                    } 
                 }
            $incree++;
    ...
    foreach($make_dynamic_main_fields as $fields){
        ...
        $make_dynamic_fields	=  $this->diamondmodel->getdata_any_table_distinct_order_where('mall_elady_products', array('product_category'=>'Bags'), 'product_sub_category', 'id');
		//print_r($make_dynamic_fields);
        foreach($make_dynamic_fields as $fields){
            if($fields['product_sub_category']){
        ?>
       $(".M<?= $incree ?>_<?= str_replace("'",'_',str_replace(' ','_',$fields['product_sub_category'])) ?>").click(function(){  dataTable.draw(); });
        <?php 
            }
        } 
    $incree++;
