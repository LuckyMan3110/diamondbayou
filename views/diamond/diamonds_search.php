						...
						<td width="100%" style="vertical-align: top;"> 
							...
							<div>
								<table id="clarity_table_new_list" style="width:100%;">
									...
									<tr>
										<td align="left"><input type="hidden" value="<?php echo isset($clarity_start)?$clarity_start:''; ?>" id="claritymin" class="w50px" onchange="modifyresult('claritymin',this.value)"></td>
										<td align="right"><input type="hidden" value="<?php echo isset($clarity_end)?$clarity_end:''; ?>" id="claritymax" class="w50px" onchange="modifyresult('claritymax',this.value)"></td>
									</tr>
								</table>
								<table width="100%">
									<tr>
						...
						<td width="100%" style="vertical-align: top;"> 
							...
							<div>
								<table id="color_table_new_list" style="width:100%;">
									...
									<tr>
										<td align="left"><input type="hidden" value="<?php echo isset($color_start)?$color_start:''; ?>" id="colormin" class="w50px" onchange="modifyresult('colormin',this.value)"></td>
										<td align="right"><input type="hidden" value="<?php echo isset($color_end)?$color_end:''; ?>" id="colormax" class="w50px" onchange="modifyresult('colormax',this.value)"></td>
									</tr>
								</table>
								<table width="100%">
									<tr>
		...
		<div class="col-sm-4 result_page_left">
			...
			<div class="result_page_content">
				...
				<div class="filter_block_section">
					...
					<label><span><input name="diamond_type" type="checkbox" id="diamond_type_V" class="diamond_type_V filtrCut " value="V" /></span>
                        <span><b>V Diamond Database</b> (<?php echo getCountResult('dev_index', 'vdbapp_id', '99999'); ?>)</span></label>
                    </div>
                    <div class="fiter_item_row">
                        <label><span><input name="diamond_type" type="checkbox" id="diamond_type_S" class="diamond_type_S filtrCut " value="S" /></span>
                        <span><b>S Diamond Database</b> (<?php echo getCountResult('dev_index', 'vdbapp_id', '0'); ?>)</span></label>
                    </div>
                    <div class="bottom_line"></div><br>
                    <div class="clear"></div>
                </div>
...
$( document ).ready(function() {
	var dataTable = $('#diamond_grid').DataTable({
		...
		"ajax":{
			...
			type: "post",
			'data' : function(data){
				<?php foreach($resultClr as $colr){ ?>console.log('<?php echo $colr['shape']?>');debugger;
				...
				var diamond_type_V	= '';
				if ($('#diamond_type_V').is(":checked")){ diamond_type_V = $('#diamond_type_V').val(); }
				var diamond_type_S	= '';
				if ($('#diamond_type_S').is(":checked")){ diamond_type_S = $('#diamond_type_S').val(); }
				...
				data.diamond_type_V	 = diamond_type_V;
				data.diamond_type_S	 = diamond_type_S;
	...
	$('.diamond_type_V').click(function(){  dataTable.draw(); });
	$('.diamond_type_S').click(function(){  dataTable.draw(); });
...
function searchDiamondFunc(key, value){
	...
	if(key == 'diamond_type'){
		$( ".filtrCut" ).prop( "checked", false );
		$(".filtrCut").removeClass("active-reactectgular");
        var checkBoxes = $("#diamond_type_"+value);
        checkBoxes.prop("checked", !checkBoxes.prop("checked"));
        $(".diamond_type_"+value).toggleClass('active-reactectgular');
    }
    if(key == 'clarity'){ 
        $( "#btn-reload" ).trigger( "click" );
    }
...
$( "#color_slider_range" ).slider({
	...
	min: 0,
	max:90,
	values: [ <?php if(isset($_GET['color']) && $_GET['color']){ echo $_GET['color']; }else{ echo 0; } ?>, 90],
	step: 10,
	slide: function( event, ui ) {
...
if(!empty($segment)){
	if(!empty($segment[4])){
		$srchShape = isset($segment[4])?$segment[4]:'';
	}else{
