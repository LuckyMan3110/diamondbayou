...
<div class="panel-container--3-uH2">
	...
	<?php /* <div>
		<div style="display:flex;width:100%;margin-bottom:20px;position:relative">
			...
			<div data-qa="style-filter" class="single-button-container--3t7nn <?php if($srchShape == 'stackable' || $srchShape == 'stackable-style' || $srchShape == 'stackable-rings'){ echo 'filteractive'; } ?>">
				<div class="single-image--12J3l icon-tension font-competability"></div>
				<span class="label--3ytQP">Sculptural</span>
				<input type="hidden" class="setting_style" id="setting_style" value="Sculptural" <?php if($srchShape == 'stackable' || $srchShape == 'stackable-style' || $srchShape == 'stackable-rings'){ echo 'checked="checked"'; } ?>>
			</div>
...
$( document ).ready(function() {
	var dataTable = $('#ring_grid').DataTable({
		...
		"ajax":{
			url :"<?php echo SITE_URL; ?>diamonds/engagement_ring_style_realtime/",
			type: "post",
			'data' : function(data){
				/* Amount */
