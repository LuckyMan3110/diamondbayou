...
table#color_table_new_list tbody tr td{padding:10px 1px 10px 15px;text-align:right}
...
function getCountResult($getTable,$fieldName,$value){
	$CI =& get_instance();
	$CI->db->where( array($fieldName => $value,'is_lab_diamond' => '1', 'fcolor_value' => '') );
	$CI->db->from($getTable);
...
<div class="sliderFilters row-fluid">
	<div class="filterCols col-sm-12 filter_show_hide" style="padding:15px 30px;">
		...
		<div class="col-sm-6">
			<table width="100%" class="clrtable">
				<tr>
					...
					<td width="100%" style="vertical-align: top;"> 
						...
						<div>
							<table id="color_table_new_list" style="width:100%;">
								...
								<tr>
									<td align="left"><input type="hidden" value="<?= isset($color_start)?$color_start:''; ?>" id="colormin" class="w50px" onchange="modifyresult('colormin',this.value)"></td>
									<td align="right"><input type="hidden" value="<?= isset($color_end)?$color_end:''; ?>" id="colormax" class="w50px" onchange="modifyresult('colormax',this.value)"></td>
								</tr>
							</table>
							<table width="100%">
								<tr>
...
<section class="main_content_bg">
	<div class="container set_col_pading">
		...
		.hover-jewelery-img-area{padding:5px 0;text-align:center}
		...
		img.detl-img2{width:182px;opacity:0;position:absolute;top:0;left:30px;height:186px;text-align:center}
		img.detl-img2:hover{width:182px;opacity:1;position:absolute;top:0;left:30px;height:186px;text-align:center}
...
$( document ).ready(function() {
	var dataTable = $('#diamond_grid').DataTable({
		"serverSide": true,
		'processing': true,
		"lengthMenu": [[15, 21, 32, 50, 100, 200], [15, 21, 30, 32, 50, 100, 200]],
		"pageLength": 21,
		'oLanguage': {"sSearch": "Enter Sku Number:", sProcessing: "<div id='loader'></div>"},
		"ajax":{
			url :"<?php echo SITE_URL; ?>diamonds/lab_diamonds_search_realtime/",
			type: "post",
			'data' : function(data){
				<?php foreach($resultClr as $colr){ ?>
...
if(!empty($segment)){
	...
	<?php
	}elseif($srchShape == 'cushion-ldcut'){ 
	?>
		<script>
		$(window).on('load', function (){
	...
	<?php
	}elseif($srchShape == 'emerald-ldcut'){ 
	?>
		<script>
		$(window).on('load', function (){
	...
	<?php
	}elseif($srchShape == 'heart-ldshaped'){ 
	?>
		<script>
		$(window).on('load', function (){
	...
	<?php
	}elseif($srchShape == 'marquise-ldcut'){ 
	?>
		<script>
		$(window).on('load', function (){
	...
	<?php
	}elseif($srchShape == 'pear-ldshaped'){ 
	?>
		<script>
		$(window).on('load', function (){
	...
	<?php
	}elseif($srchShape == 'radiant-ldcut'){ 
	?>
		<script>
		$(window).on('load', function (){
	...
	<?php
	}elseif($srchShape == 'princess-ldcut'){ 
	?>
		<script>
		$(window).on('load', function (){
