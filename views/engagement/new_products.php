                      ...
                      <li class="header last">
                        <div>Category</div>
                                   ...
                                   <ul class="topFilters">
									 ...
									 if($arr_length==3)
									   $b_link = "engagement/newstullerinventory";
									 else{  
							...
							foreach($all_sub_category as  $item){ 
								   $sub_cat = $this->jewelrymodel->getstuller_sub_level_new($item['items'],$level-2);
								   if($item['items']!="")
								{
								 
								 ?>
				           <li class="qTipMegaMenu" style=""> 
									<a aria-describedby="ui-tooltip-1" class=" sub " href="<?php echo config_item('base_url')."engagement/newproducts/". $link. $item['items']; ?>" style="color:#55697F;" >
											   ...
											   { 
												?>
												  <li class="firstLevelChildren hasChildren">
													    <a href="<?php echo config_item('base_url')."engagement/newproducts/".$link.$item['items']."/".$c_cat['items']; ?>" ><span style='color:black;'><?php echo $c_cat['items'];?></span></a>
													     <?php
													    $child_cat = $this->jewelrymodel->getstuller_sub_level_sub_new($item['items'],$c_cat['items'],$level-1);
														 ...
														 ?>
													      <ul class="secondLevelChildren">
															        <?
																	  ...
																	  ?>
															          <li>  <a href="<?php echo config_item('base_url')."engagement/newproducts/".$link.$item['items']."/".$c_cat['items']."/".$s_c_cat['items']; ?>"><?  echo $s_c_cat['items'];?></a></li>                         
														           <?}?>
														  </ul>
														 <?}?> 
												  </li>
...
if(count($products) > 0) {	
       ///print_r($products);
		foreach ($products as $row) {
					    ...
					    $image1 = $site_link  ."stuller/mountings2/images/".trim($row["thumb1"]);                      
					//if (fopen($image1, "r")) {	
						$update_price = $this->jewelrymodel->get_update_price($row['RetailPrice'],'dev_helix_stuller_prices');
				        $title='';
				        if(!empty($row["Level1"])){
...
//img1,	ImagePath,StoneMapImageFile,Level1,	Description,ItemNumber,	RetailPrice	,PrimaryMetalComposition,Quality
	$returnhtml .= '<div style=" height: 264px; margin-left: 15px;margin-top: 30px;width: 170px;" class="floatl ringbox txtcenter" >
							...
							?>
                               ...
                               <a href="javascript:void(0)" onclick="showdetails_new('ff','<?php echo $lot;?>')"  >
                                                                            
							    		 <center>
								    		 <div class="ringtile">
								    		 <!--<img style=" width:100px;height:150px;"  src="<?= $site_link  ."stuller/mountings2/images/".trim($row["thumb1"]);?>" width="70" ><br>-->	
                                                                                     <img style=" width:100px;height:150px;"  src="<?= $site_link  ."stuller/".  str_replace('\\','/',strtolower(trim($row["img1"])));?>" width="70" ><br>
								    		 </div>
							    		 </center>
							  </a>                                   
         				<?	
						$returnhtml='<div style="color:#000000;" class="txtsmall">Retail Price: $'.number_format($update_price,2).'</div>
	...
	// <a  style="color:#000000;"  href="'.$href.'" class="search2">View Details</a>'
	 ...
	 $returnhtml="";
	 ?>
                <a  style="color:#000000;"  href="javascript:void(0)"   class="search2"  onclick="showdetails_new('ff','<?php echo $lot;?>')" >View Details</a>
	        </div>
                    <?  $i = $i + 1;
