...
<script language="javascript" src="<?php echo SITE_URL; ?>js/common_function.js" type="text/javascript"></script>
    <div class="mainwrap"><br>
        ...
        <div class="uper_detail_block">
            ...
            <div class="further_dtcols">
                <span>Metal</span>
                <span><?php echo $rowsring['metal_purity']; ?></span>
            </div>
           ...
           <?php
             ...
             if( !empty($rowsring['ring_size']) ) {
                $ringsize_options .= '<div class="further_dtcols">
                                        <span>Ring Size</span>
                                        <span>'.$rowsring['ring_size'].'</span>
                                    </div>';
             }
             if( !empty($rowsring['ringtype']) ) {
                $ringsize_options .= '<div class="further_dtcols">
                                        <span>Ring Type</span>
                                        <span>'.$rowsring['ringtype'].'</span>
                                    </div>';
             }
...
<!--                <span>Retail Price</span>
            ...
            <div class="further_dtcols">
               ...
               <div class="further_dtcols">
                <span>Our Price</span>
                <span class="price_label prices_contact">Please Call <?php echo CONTACT_NO; ?> for price <?php //echo _nf($sales_price, 2); ?></span>
            </div>
               <div class="clear"></div><br>
               <div class="earnpoints">Earn <span>6118</span> D-Stern Reward Points</div>
               <hr class="horizontal_line" />
            <?php if( in_array($rowsring['category'], $rings_allowed_id) ) { ?>
            ...
            <?php } ?>
               ...
               <div class="detail_botom_text">
                   ...
                   <span></span>
                   <span><a href="<?php echo SITE_URL.'account/account_wishlist/'.$rowsring['stock_number']; ?>"><img src="<?php echo IMGSRC_LINK; ?>addto_wish_ic.jpg" alt="addto_wish_ic"></a></span>
               </div>
               <div class="clear"></div><br><br>
                    ...
                    <input type="hidden" name="prid" value="<?php echo $rowsring['stock_number'];?>">
                    ...
                    <input type="hidden" name="txt_addoption" value="sterncollection" />
                    <input type="hidden" name="unique_pagelink" value="<?php echo $page_link; ?>" />
                    <input type="hidden" name="vendors_name" value="Heart and Diamond Collection" />
                    <div class="leftbtn_block">
                       <div class=""><a href="#javascript" onclick="addcartsubmit('<?php echo $buynow_link; ?>')" id="addtoshopping"><img src="<?php echo SITE_URL; ?>images/buy-now.jpg" alt="now"/></a><br /></div> 
                    </div>
            ...
            <div class="tabsdata">
                <div id="product_details">
                    <div class="details_tab_left">
                        The Heart and Diamond Collection is mostly a one of a kind, limited edition or special order. All the jewelry shown is subject to prior sale. The designs that have center diamonds or color stones are priced for the item shown. These designs are available as a special order.  Diamonds can be duplicated as the original so the price will be the same. Every attempt will be made to match all other types of stones for color, clarity, and size, but due to the nature of some color stones and the design of the jewelry, not all will be exact as shown on the website. When this occurs, you will be notified. I will show you a  replacement that in my opinion will closely resemble the original. This will be done by computer skype video for an approval by you the buyer. Some of the designs will be available for your center stone. Most designs can be remade the same to fit your stone if required or I will sell you the design as a semi-mounting by special order. If this is your request then you will need to send a photo of your stone and measurements. You can also call me directly <?php echo CONTACT_NO; ?>. I oversee all the stages of manufacturing for the Heart and Diamond Collection and special orders. All special orders will take three to six weeks for delivery. You can e-mail me to answer any questions via the website.
 <br><br>
CONDITIONS OF BUSINESS ( ADD THIS IF IT IS NOT ALREADY DONE)<br><br>
Special orders require a 60% deposit and the balance paid via the internet site before delivery. There are no refunds for any special orders.<br>
 Financing is available but the jewelry ordered must be paid in full on the day of the order.
                       <?php //echo $item_desc; ?>
                    </div>
                ...
                <?php
                    $itemInformation = '';
                    if( !empty($rowsring['metal_purity']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Metal Purity</span>
                                            <span>'.$rowsring['metal_purity'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['item_title']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Item Title</span>
                                            <span>'.$rowsring['item_title'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['vendor_name']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Vendor Name</span>
                                            <span>'.$rowsring['vendor_name'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['vendor_sku']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Vendor SKU</span>
                                            <span>'.$rowsring['vendor_sku'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['item_sku']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Item SKU</span>
                                            <span>'.$rowsring['item_sku'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['ring_size']) ) {  
                    ...
                    if( !empty($rowsring['measurements']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            ...
                                            <span>'.$rowsring['measurements'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    ...
                    if( !empty($rowsring['length']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Length</span>
                                            <span>'.$rowsring['length'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['width']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Width</span>
                                            <span>'.$rowsring['width'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['chain_type']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Chain Type</span>
                                            <span>'.$rowsring['chain_type'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['clasp_type']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Clasp Type</span>
                                            <span>'.$rowsring['clasp_type'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['gemstone_type']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Gemstone Type</span>
                                            <span>'.$rowsring['gemstone_type'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['carat']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Carat</span>
                                            <span>'.$rowsring['carat'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
                    if( !empty($rowsring['shape']) ) {  
                        $itemInformation .= '<div class="item_rows">
                                            <span>Shape</span>
                                            <span>'.$rowsring['shape'].'</span>
                                            <div class="clear"></div>
                                        </div>';
                    }
            ...
            <div class="popular_rings">
                    ...
                    <?php echo $more_collection_listings; ?>
                    <div class="clear"></div><br>
                </div>
            <div class="clear"></div><br><br>
        </div>
    ...
    <form name="askFriendForm" id="askFriendForm" method="post" action="#">
      <div class="p_content">
        ...
        <div class="formArea">
          ...
          <div class="fieldBlock">
            <input type="hidden" name="details_link" id="details_link" value="<?php echo 'davidsternrings/collection_detail/'.$rowsring['stock_number'];?>">
