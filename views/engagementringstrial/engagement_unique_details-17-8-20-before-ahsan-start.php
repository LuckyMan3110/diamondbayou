...
<style>
#offerpopup input[type=text], select, textarea{padding: 8px !important;height: 35px;margin:0px !important;}
.container{padding:20px;margin:0px;width:100%}
...
<div class="diamondViewDetail container2 uniqueRingDetail">
	...
	if($row_cate['parent_cid'] == 63 OR $row_cate['parent_cid'] == 69 OR $row_cate['parent_cid'] == 67 OR $row_cate['parent_cid'] == 57 OR $row_cate['parent_cid'] == 71 OR $row_cate['parent_cid'] == 73){
		...
		if(isset($_GET['carat'])){
			if(isset($_GET['dev_index_id'])){
				$where_dev_index	= array('uid' => $_GET['dev_index_id']);
				...
				$where_dev_index		= array('carat' => $_GET['carat'], 'price !=' => '', 'color !=' => '');
				$get_dev_index_all_info	= $this->catemodel->getdata_any_table_limit_order_by_where_like('dev_index', $where_dev_index, '1', '0', 'uid', 'ASC', $additional_stones_first_item['1'], 'Meas');

				$get_Meas_value     = $get_dev_index_info['0']->Meas;

				$get_uid_find_dia   = $get_dev_index_info['0']->uid;
				...
				if($_GET['carat'] == $additional_stones_new){
					$where_dev_index	= array('carat' => $_GET['carat'], 'price !=' => '', 'color !=' => '');
					$get_dev_index_info	= $this->catemodel->getdata_any_table_limit_order_where('dev_index', $where_dev_index, '100000', '0', 'price');
				}else{
					$where_dev_index	= array('carat' => $_GET['carat'], 'price !=' => '', 'color !=' => '');
					$get_dev_index_info	= $this->catemodel->getdata_any_table_limit_order_by_where_like('dev_index', $where_dev_index, '100000', '0', 'price', 'ASC', $dev_index_us_meas, 'Meas');
				}
				...
				if( count($get_dev_index_info) > 0 ) {
					foreach( $get_dev_index_info as $rows ) {
						...
						if(in_array($rows->color, $white_color_array) AND $dev_index_us_meas == $dev_index_meas){
							$get_Meas_value     = $rows_dev_index_info->Meas;
							$get_uid_find_dia   = $rows_dev_index_info->uid;
							$get_diamond_price  = $rows_dev_index_info->price;
							$get_cert_no        = $rows_dev_index_info->Cert_n;
							$get_color          = $rows_dev_index_info->color;
							$get_clarity        = $rows_dev_index_info->clarity;
							$get_shape          = $rows_dev_index_info->shape;
							$get_Stock_n        = $get_dev_index_info['0']->Stock_n;
							$check_data++;
						}
					}
			...
			$center_stone_solid		= explode('-', $additional_stones_new);
			$center_stone_solid1	= $center_stone_solid['1'];
			$center_stone_solid2	= explode('x', $center_stone_solid1);

			if($center_stone_solid2['1']){
				$center_stone_solid11        = $center_stone_solid2['1'];
				$center_stone_solid22        = explode('.', $center_stone_solid11);
				if($center_stone_solid22['1']){
					$center_stone_solid_data  = $center_stone_solid22['0'];
				}else{
					$center_stone_solid_data  = '0.'.$center_stone_solid2['0'];
				}
				$center_stone_solid_data  = (float)$center_stone_solid_data;
			}else{
				$center_stone_solid11	= $center_stone_solid1;
				$center_stone_solid22	= explode('.', $center_stone_solid11);

				if($center_stone_solid22['1']){
					$center_stone_solid_data  = (float)$center_stone_solid11;
				}else{
					$center_stone_solid_data  = (int)$center_stone_solid1;
				}
			}

			if($center_stone_solid_data){
				$where_dev_index	= array('price !=' => '', 'color !=' => '', 'carat' => $center_stone_solid_data);
			}else{
				$where_dev_index	= array('price !=' => '', 'color !=' => '', 'carat' => 1);
			}
			$get_dev_index_info	= $this->catemodel->getdata_any_table_limit_order_by_where_like('dev_index', $where_dev_index, '1', '0', 'uid', 'ASC', $center_stone_solid_data, 'carat');

			if( empty($get_dev_index_info) ){
				$where_dev_index	= array('carat' => $additional_stones_new, 'price !=' => '', 'color !=' => '');
				$get_dev_index_info	= $this->catemodel->getdata_any_table_limit_order_where('dev_index', $where_dev_index, '1', '0', 'price');
			}
	...
	if(isset($_GET['carat'])){
		...
		$get_carat_value_d_i_NEW  =  $rowsring['stone_weight'] + $get_carat_value_d_i;

		if($get_carat_value_d_i_NEW == 0){
			$get_carat_value_d_i = 1;
		}
			...
			<div class="set_steps_bar" id="builder_stpes_bar"><img src="<?php echo SITE_URL; ?>img/heart_diamond/ring_builder_steps.jpg" alt="ring_builder_steps" /></div>
				<div class="detail_column">
						...
						<div class="detail_right col-sm-6">
							...
							<div class="learn_about">
								<div class="further_dtcols" style="display:none;">
									...
									<span><strike>$ <?php 
									echo _nf($retail_price+$get_diamond_price, 2); ?></strike> <span>save <?php echo $saving_percent; ?>%</span></span>
								</div>    
							...
							<div class="metal_list_box">
								...
								<ul class="set_metals_list">
									<li>
										<a href="<?php echo SITE_URL; ?>selection/engagement-ring-detail/<?php echo $rowsring['ring_id'];?>/1/14KP/">
										<img src="<?php echo SITE_URL; ?>img/heart_diamond/metal_icons_2.jpg" alt="metal_icons_2" />
										</a>
									</li>
									<li>
										<a href="<?php echo SITE_URL; ?>selection/engagement-ring-detail/<?php echo $rowsring['ring_id'];?>/1/18KP/">
										<img src="<?php echo SITE_URL; ?>img/heart_diamond/metal_icons_5.jpg" alt="metal_icons_5" />
										</a>
									</li>
									<li>
										<a href="<?php echo SITE_URL; ?>selection/engagement-ring-detail/<?php echo $rowsring['ring_id'];?>/1/PLAT/">
									...
									if($row_cate['parent_cid'] == 63 OR $row_cate['parent_cid'] == 69 OR $row_cate['parent_cid'] == 67  OR $row_cate['parent_cid'] == 57 OR $row_cate['parent_cid'] == 71 OR $row_cate['parent_cid'] == 73){
										...
										for($i=1; $i <= 4; $i++) {
											...
											$diamond_carat_list .= '<li class="'.$active_carat.'">
												<a href="'.SITE_URL.'selection/engagement-ring-detail/'.$rowsring['ring_id'].'/1/'.$ring_metal_type.'/?carat='.$carat_number.'">
								...
								<div class="price-with-button force-wrap" data-smart-wrap="true">
									<div class="price-display">
										<div class="regular-price" style="display:none;">
											<span class="price" itemprop="price" content="<?php echo _nf($rowsring['priceRetail']+$get_diamond_price, 2) ; ?>">$<?php echo _nf($rowsring['priceRetail']+$get_diamond_price, 2); ?></span>
										</div> 
										...
										<div style="text-align:right;font-size:17px;">
											...
											if($row_cate['parent_cid'] == 63 OR $row_cate['parent_cid'] == 69 OR $row_cate['parent_cid'] == 67 OR $row_cate['parent_cid'] == 57 OR $row_cate['parent_cid'] == 71 OR $row_cate['parent_cid'] == 73){
												...
												<a href="#javascript" onclick="setHDIndexDiamondList('<?php echo $get_uid_find_dia; ?>', '<?php echo $get_shape_find_dia; ?>', '<?php echo $get_color_diamond; ?>', '14k_gold', 0, '1920', '<?php echo $rowsring['ring_id'];?>', '<?php echo $rowsring['additional_stones']; ?>')<?php// echo $load_center_diamond; ?>">(Need More Center Diamond Options? Click Diamond Finder &trade;)</a>
											<?php } ?>
										</div>
									</div>
								</div>    
					...
					<div id="details-and-purchase">
						...
						<div class="detail_bk_row first_item_d_rin">
							<div class="detail_left_cols">Center Diamond Stock Number</div><div class="detail_right_cols"><a href="<?php echo base_url();?>/diamonds/diamond_detail/<?php echo $get_dev_index_info['0']->lot;?>"/false/"><?php echo $get_Stock_n; ?></a></div><div class="clear"></div>
						</div>
						...
						<?php if($row_cate['parent_cid'] == 63 OR $row_cate['parent_cid'] == 69 OR $row_cate['parent_cid'] == 67 OR $row_cate['parent_cid'] == 57 OR $row_cate['parent_cid'] == 71 OR $row_cate['parent_cid'] == 73){
							...
							<?php if($rowsring['additional_stones'] == ''){
								...
								<div class="detail_bk_row">
									<div class="detail_left_cols">Center Diamond Shape</div>
									<div class="detail_right_cols">
										<?php 
										$additional_stones_ex = explode('/', $rowsring['additional_stones']);
							...
							<div class="detail_bk_row">
								...
								<div class="detail_right_cols">ROUND</div>
								<div class="clear"></div>
							</div>
							<div class="detail_bk_row">
								<div class="detail_left_cols">Side Diamond Carats</div>
								<div class="detail_right_cols"><?php echo $rowsring['stone_weight']; ?></div>
								<div class="clear"></div>
							</div>
							<div class="detail_bk_row">
								<div class="detail_left_cols">Center Diamond Carat</div>
								<div class="detail_right_cols">
									<?php
									if(isset($_GET['carat'])){
										echo $_GET['carat'];
							...
							<div class="detail_bk_row">
								<div class="detail_left_cols">Total Carat Weight</div>
								<div class="detail_right_cols">
									<?php
									if(isset($_GET['carat'])){
										echo $rowsring['stone_weight'] + $_GET['carat'];
							...
							<div class="detail_bk_row">
								<div class="detail_left_cols">Center Loose Diamond Price</div><div class="detail_right_cols">$<?php echo _nf($get_diamond_price, 0); ?></div><div class="clear"></div>
							</div>
							<div class="detail_bk_row">
								...
								?> Price</div><div class="detail_right_cols">$<?php echo _nf($rowsring['priceRetail']+$get_diamond_price, 2) ; ?></div><div class="clear"></div>
							</div>
							<?php
							$item_listing = array(
                ...
                <div class="detail_bk_left">
                    <div class="detail_inner_bk">
                        ...
                        <div>
                            ...
                            <div class="prices_label">$<?php echo _nf($rowsring['priceRetail'], 2) + $get_diamond_price; ?></div><br />
                  ...
                  <div class="other_link_list">
                    ...
                    <div class="other_reports_link">
                        <ul>
                          ...
                          <li><a href="http://www.gia.edu/cs/Satellite?reportno=2145710367&amp;c=Page&amp;childpagename=GIA%2FPage%2FReportCheck&amp;pagename=GIA%2FDispatcher&amp;cid=1355954554547&amp;encryptedString=7CD682F48A4AC0441FEEC95403BDAA3C" target="_blank">Verify Lab Report</a></li>
                      </ul>
                    </div>
				</div>
                <div class="clear"></div><br>
            ...
            <div class="topbar_cart_left">
                <?php 
                    ...
                    <?php }else{?>
                        ...
                        <span style="font-size:30px;">$<?php echo _nf($rowsring['priceRetail']+$get_diamond_price, 2) ; ?></span> <a href="#" class="addtocart_btn" onclick="addcartsubmit('<?php echo $buynow_link; ?>')">Add To Cart</a>
