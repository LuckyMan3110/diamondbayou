				...
				<div id="accordion-product-details" class="product-details-nav-product-details accordion-item accordion-item-mutually-exclusive accordion-item-default accordion-item-standard accordion-item-medium accordion-item-small _open">
					<div class="detail_center">
						...
						<div class="tz-gallery-collection">    
							...
							<?php 
							if(!empty($rowsring['product_gallery_imges'])){
								...
								if(!empty($gallery_imgs)){
									foreach($gallery_imgs as $gallery_img1){
										...
										?>
										<a href="javascript:void(0);" onclick="ch_img1('<?php echo $gal_org;?>')">	<img src="<?php echo $gallery_img1; ?>" style="width:55px;height:55px;" alt="<?php echo $rowsring['product_name']; ?>" /></a>
										<?php
									}
								}else{
								?>
							...
							if(!empty($rowsring['video_url'])){ ?>
								<a id="video" href="javascript:void(0);" video-url="<?php echo $rowsring['video_url'];?>"><img src="<?php echo base_url().'collection_images/video-btn.png';?>" style="width:55px;height:55px;"></a>
							<?php } ?>
                        </div>        
								...
								<div class="right-half">
									<div id="setting-information-table">
										<div class="detail-table">
											<?php
											if($rowsring['product_specification'] != ''){ 
												$rs_strins = explode('<th class="col label" scope="row">',$rowsring['product_specification']);
												$rs_strins_lab = explode('<th class="col label" scope="row">',$rowsring['product_specification']);
												if(!empty($rs_strins)){
													$i1 = 0;
													foreach($rs_strins as $rs_sting_1){
														$rs_string_2 = explode('</th>',$rs_strins_lab[$i1]);
														$rs_sting_1 = explode('<td class="col data" data-th="'.$rs_string_2[0].'">',$rowsring['product_specification']);
...
.addtowishlist_btn{background:#666;max-width:195px;width:100%;padding:16px 10px;text-align:center;text-transform:capitalize;display:inline-block;color:#fff;margin-top:17px}
.addtowishlist_btn:hover,.addtowishlist_btn:hover{background:#006495}
.topbar_cart_left{float:left;width:83%}
</style> 
