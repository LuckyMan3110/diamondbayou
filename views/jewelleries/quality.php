...
function showdetail(id){
	urllink = base_url+'site/qualitydetail/'+id+'/';	
			$.ajax({
                 type: "POST",
                 url:urllink,
...
<tr><td><table><tr><td style="color:#FFFFFF"><strong>Cyjewelers American Collection I</strong></td></tr>
...
<?php 
if($this->uri->segment(3)=="Ring" || $this->uri->segment(3)=="Mens" || $this->uri->segment(3)=="Earring" || $this->uri->segment(3)=="Pendant" || $this->uri->segment(3)=="Necklace" || $this->uri->segment(3)=="Bracelet" )
{
...
?>
<tr><td colspan="4"><a href="<?php echo config_item('base_url') ?>jewelleries/getstullerfur/<?php echo $journalName; ?>/<?php echo $this->uri->segment(3);?>/"><?php 
 echo $allsub['Metal_Desc'];
    ?></a></td></tr>
<?php }}?>
</table></td><td valign="top" align="right">
<table>
<tr><td style="color:#FFFFFF"><strong>Cyjewelers American Collection II</strong></td></tr>
<?php 
...
foreach($result as $allsub)// returns an object of the first row
{
	...
	?>
<tr><td colspan="4"><a href="<?php echo config_item('base_url') ?>jewelleries/getmystullerfurwithsub/<?php echo $journalName; ?>/<?php echo $this->uri->segment(3);?>/"><?php 
 echo $allsub['MerchandisingCategory1'];
    ?></a></td></tr>
<?php }?>
...
?>
<div class="panels-1">
<div class="jtitle" style="color:White"><?php echo $records['result'][$i]['Metal_Desc'];?></div>
         <div class="panel-1">
            <!--   <a href="javascript:;" onclick="showdetail(<?php //echo $records['result'][$i]['stock_number']; ?>)" class="" style="width: 325px;">-->
			 ...
			 ?>
                  <img src="<?php echo $src;?>" alt="<?php echo $records['result'][$i]['Metal_Desc'];?>" class="first" style="height:210px;width:325px;">
              </a>
          </div>
		  <div class="jprice" style="color:White">Best:$<?php echo number_format($records['result'][$i]['MSRP'],2);?></div>
		<?php if($records['result'][$i]['ezstatus'] == true){ ?>
		<div class="jprice" style="color:White">3EZ:$<?php $ez3 = $records['result'][$i]['MSRP']+((12*$records['result'][$i]['MSRP'])/100); echo number_format($ez3,2); ?></div>
		<div class="jprice" style="color:White">5EZ:$<?php $ez5 =  $records['result'][$i]['MSRP']+((17*$records['result'][$i]['MSRP'])/100); echo number_format($ez5,2); ?></div><?php } ?>

		  <div class="jdetail"> <a href="javascript:;" onclick="showdetail(<?php echo $records['result'][$i]['qg_id']; ?>)">View Detail</a>
