...
<?php 
 ...
 $statement = "SELECT DISTINCT `MerchandisingCategory2` FROM `dev_stuller` WHERE `MerchandisingCategory1` LIKE '%".str_replace('_', ' ', $this->uri->segment(3))."%'";

$query =$this->db->query($statement);
$result = $query->result_array();
foreach($result as $allsub)// returns an object of the first row
{
	$journalName = preg_replace('/\s+/', '_', $allsub['MerchandisingCategory2']);
?>
<tr><td colspan="4"><a href="<?php echo config_item('base_url') ?>jewelleries/getmysubstullerfur/<?php echo $journalName; ?>/"><?php 
 echo $allsub['MerchandisingCategory2'];
    ?></a></td></tr>
<?php }?>
<tr>
