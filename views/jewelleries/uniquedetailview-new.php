...
<div class="leftCl">
  <?php
	//echo current_url();
		...
		//print_r($getUrl);
		echo accordian_left_menu($style_group);
	?>
</div>
<div class="rightCl">
  <div class="pgSt">
    <div class="bread-crumb brbg">
      <ul>
        ...
        <li><a href="<?php echo config_item('base_url') ?>site/jewelary/engagement">Engagement Rings</a></li>
        <li><a href="<?php echo config_item('base_url') ?>jewelleries/engagementQuality/"><?php echo $seting_type; ?></a></li>
      </ul>
    </div>
    <br />
  </div>
  <div class="contentTb">
	...
	$ringDescription = 'Yadegardiamonds.com stunning '.$seting_type.' Style diamond semi mount ring can be yours for <span id="viewDyPrice">$'.$netRingPrice.'</span>. This ring does not include the diamond. Diamonds are sold seperately.';
    ...
    <form id="form1" name="form1" method="post" action="">
        <table class="contb_bk">
          <tr>
            <td><div class="whHead"> <span class="dwar-icon"></span> <?php echo $details['name']; ?> </div>
              <br />
              <div class="textList">
                <label>Setting Type:&nbsp;&nbsp;</label>
                <span><?php echo $seting_type; ?></span> </div>
              <div class="textList">
                <label>Product ID:&nbsp;&nbsp;</label>
                <span><?php echo $details['style_group']; ?></span> </div>
              <div class="textList">
              <?php
                if($cuprice > 0) {
                    echo '<label>Price:&nbsp;&nbsp;</label>
                <span id="vieDyPrice">$'.$netRingPrice.'</span>';
                } else {
                    echo '<label>Call: 415.626.5035</label>';
                }
				$image_url = 'http://www.uniquesettings.com'.$details['image'];
              ?>
                </div>
              <br />
              <br />
            ...
            <td colspan="2"><img src="<?php echo $image_url; ?>" width="400" alt="<?php echo $details['name']; ?>" /> 
              <!--<img src="<?php echo config_item('base_url') ?>images/scroll_gl.jpg" />--></td>
          </tr>
          <tr>
            <td><div class="whHead"> <span class="dwar-icon"></span> Engrave Setting: </div>
              <br />
            ...
            <td><div class="engra_text social_box">
                <div class="textList">
                  ...
                  <span>
                  <select name="item_metaltp" id="cmb_metaltype" onchange="calcRingPrice()" class="dropdw_bx">
                    <?php
                        $metalType = '';
                        if(count($uniqueprice) > 0) {
                        foreach($uniqueprice as $row_metal) {
							$metal_value = str_replace(' ','_', $row_metal['matalType']);
                        $metalType .= '<option value="'.$metal_value.'">'.$row_metal['matalType'].'</option>';
                        }
                        } else {
                ...
                <div class="textList">
                  ...
                  <span>
                  <select name="item_rsize" id="cmb_ringsize" onchange="calcRingPrice()" class="dropdw_bx">
                    <?php
                        $getRingSize = '';
                        if(count($ring_sizes) > 0) {
                        foreach($ring_sizes as $row_rsize) {
							$ring_value = str_replace(' ','_', $row_rsize['ringSize']);
							$ring_value = str_replace('/','-', $ring_value);
							
                        $getRingSize .= '<option value="'.$ring_value.'">'.$row_rsize['ringSize'].'</option>';
                        }
                        } else {
                        $getRingSize .= '<option value="">Ring Size</option>';
                        }
                        echo $getRingSize;
                    ?>
                  </select>
                  <!--<input name="txt_rsize" class="mtringSize" type="text" value="<?php echo $ring_size; ?>" />-->
                  </span> </div>
                <div class="textList"> <br />
                  <div> <a href="#" onclick="addcartsubmit('<?php echo $buynow_link; ?>')" id="addtoshopping"><img src="<?php echo config_item('base_url') ?>images/buy-now.jpg" /></a><br />
                    <br />
                    <a href="#javascript;" onclick="addcartsubmit('<?php echo $addtoring_link; ?>')" id="addtoshopping"><img src="<?php echo config_item('base_url') ?>images/select-ring.jpg" /></a><br />
                  </div>
                  <div class="clear"></div>
                  <br />
                  ...
                  <div id="featured2">
                    ...
                    <div class="tabBox">
                      ...
                      <div class="tabsRow">
                        ...
                        <span class="sizeBlock"><?php echo $uniqueprice[0]['matalType']; ?></span>
                        </div>
                      </div>
                      <!--<div class="tabsRow">
                  ...
                  <div id="core2" class="hide">
                    <div class="similar_prod">
                      <?php
                        ...
                        if($ring_id == $records['result'][$i]['id']) {
                            continue;	
                        }
        
                        ?>
                      <div class="prodBlock">
                        <div class="imgBlock"><a href="<?php echo config_item('base_url') ?>site/enagagementqualitydetail/<?php echo $records['result'][$i]['id']; ?>/<?php echo $unique_price.'/'.$style_group; ?>"><img src="http://www.uniquesettings.com<?php echo $records['result'][$i]['image'];?>" alt="<?php echo $records['result'][$i]['name'];?>" width="155" hight="144"></a></div>
                        <div class="prodLable"><?php echo $records['result'][$i]['name']; ?><br />
                          <span>
                          <?php
						  	if($unique_price > 0) {
								$unPrice = '$ '.$unique_price;
							} else {
