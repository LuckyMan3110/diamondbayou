<div class="content row-fluid engagement">
    ...
    <div id="uniquRingView" class="col-sm-9">
      ...
      <br />
      <div class="setpagination anotherPaginate"><ul><?php echo $pagination_links; ?></ul></div>
      <div class="clear"></div>
      <br />
     ...
     <?php
            ...
            if(count($results_rings)>0){
                foreach($results_rings as $rows_rings)
                {
                    ...
                    $ringdetail_link = config_item('base_url').'site/engagementRingDetail/'.$catgory_id.'/'.$rows_rings['ring_id'];
                    $viewRingImg = config_item('base_url').'scrapper/imgs/'.$rows_rings['ImagePath'];
                    ...
                    $ringDescription = $sitesTitle.' stunning '.$maincate_name.' '.$cates_stname.' Style '.$cates_name.' diamond ';
                    if(!in_array($getparent_cate, $notallowed_id))
                    {
                        $ringDescription .= 'semi mount ';
                    }
                    $ringDescription .= 'ring can be yours for $'.$netRingPrice.'. This ring does not include the diamond. Diamonds are sold seperately.';
      ...
      <span class="setcolr">&ndash;</span>
        ...
        <a href="<?php echo $ringdetail_link ?>">&nbsp;<img src="<?php echo $viewRingImg;?>" alt="<?php echo $rows_rings['name'];?>" width="155" hight="144"></a>
        ...
        <div class="price-product ringprice-section">
          ...
          <p class="ring-engagement"><a href="<?php echo $ringdetail_link ?>">Engagement Rings - <?php echo $rows_rings['name'];?></a></p>
          <div>
          <?php
            if($getRingPrice > 0) {
                echo 'Price: $'.$getRingPrice;
            } else {
