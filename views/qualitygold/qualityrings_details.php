...
<div id="" class="row-fluid ringsDetail">
  <div class="col-sm-3">
	...
	//echo current_url();
		...
		//print_r($getUrl);
		echo accordian_left_menu('quality');
	?>
  </div>
  <div class="col-sm-9">
    <div class="pgSt">
	...
	{
		$addtoRingBtn = '<a href="#javascript;" onclick="addcartsubmit(\''.$addtoring_link.'\')" id="addtoshopping"><img src="'.config_item('base_url').'images/select-ring.jpg"  alt="ring"/></a><br />';
	...
	{
		$addtoRingBtn = '';	
		$uniqueRingsDesc = get_site_title().' stunning '.$details['title'] . ' can be yours for <span id="viewDyPrice">$'.$netRingPrice.'</span>.';
		$viewSupliedStone = '';
	}
        $image_url = QUALITY_IMGS.$details['small_image'];  /// get stuller image function in custome_helper
    ?>
      <form id="form1" name="form1" method="post" action="">
            <div class="row-fluid"> <!-- old class: contb_bk -->
            <div class="col-sm-5">
            <div class="whHead"> <span class="dwar-icon"></span> <?php echo $details['title']; ?> </div>
              <br />
              <div class="textList">
                <label>Product ID:&nbsp;&nbsp;</label>
                <span><?php echo $details['style']; ?></span> </div>
<!--              <div class="textList">
                <?php
                if($netRingPrice > 0) {
...
<!-- <input type="radio" name="rb_pricetp" value="SP" checked="checked" />-->
              ...
              <div class="textList">
                ...
                <span class="engra_text">
                    <?php echo _nf($details['average_weight'],2); ?>
...
<!--                <div class="textList">
                ...
                <div class="textList"> <br />
                  ...
                  <div> <a href="#javascript" onclick="addcartsubmit('<?php echo $buynow_link; ?>')" id="addtoshopping"><img src="<?php echo config_item('base_url') ?>images/buy-now.jpg"  alt="buy-now"/></a><br />
                    <br />
                    <?php endif; ?>
                    ...
                    <input type="hidden" name="prodname" value="<?php echo $details['title']?>">
                    <input type="hidden" name="diamnd_count" value="">
                    <input type="hidden" name="ring_shape" value="">
                    <input type="hidden" name="ring_carat" value="<?php echo $details['average_weight'];?>">
...
<div class="textList"> <a href="<?php echo SITE_URL.'account/account_wishlist/'.$ring_id.'/add/qualityrings'; ?>"><span class="addic"></span>&nbsp;&nbsp;Add To wish List</a> </div>
            </div>
                </div>
              </div>
                ...
                <div class="list-wrap">
                  <div id="featured2">
                    ...
                    <div class="col-sm-6">
                      ...
                      <div class="tabsRow">
                        <div class="metaLeft">Item Style: <span class="sizeBlock"><?php echo $details['style']; ?></span> </div>
                      </div>
                      <div class="tabsRow">
                        <div class="metaLeft">Item Category: <span class="sizeBlock"><?php echo $prod_cate; ?></span> </div>
                      </div>
                      <?php
                      $viewSetting = '';
                    ...
                    if( !empty($details['item_length']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Item Length: <span class="sizeBlock">'.$details['item_length'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['average_weight']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Average Weight: <span class="sizeBlock">'.$details['average_weight'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['country']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Country: <span class="sizeBlock">'.$details['country'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['metal']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Metal: <span class="sizeBlock">'.$details['metal'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['product_type']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Product Type: <span class="sizeBlock">'.$details['product_type'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['jewelry_type']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Jewelry Type: <span class="sizeBlock">'.$details['Weight'].' '.$details['jewelry_type'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['material_primary']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Material Primary: <span class="sizeBlock">'.$details['material_primary'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['material_primary_color']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Material Primary Color: <span class="sizeBlock">'.$details['material_primary_color'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['material_purity']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Material Purity: <span class="sizeBlock">'.$details['material_purity'].'</span>'.'</div></div>';
                    }
                    if( !empty($details['length_of_item']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft">Length of Item: <span class="sizeBlock">'.$details['length_of_item'].'</span>'.'</div></div>';
                    }
                    
                    if( !empty($details['catalogs']) ) {  
                      $viewSetting .= '<div class="tabsRow">
                        <div class="metaLeft"><b>Catalogs</b>: <span class="sizeBlock">'.$details['catalogs'].'</span>'.'</div>
