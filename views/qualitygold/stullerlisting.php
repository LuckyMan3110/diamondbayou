    ...
    <div id="uniquRingView" class="col-sm-9">
      ...
      <div class="breakcrum_bk col-sm-6">
        <ul>
              ...
              <?php
                if( !empty($ringsCate) && $ringsCate != 1 ) {
                    echo '<li><a href="#">'.$ringsCate.'</a></li>';
                }
              ?>
            </ul>
        </div>
     ...
     <?php
            ...
            if(count($results_rings)>0){
                ...
                {
                    $stulerImage = ( !empty($rows_rings['Images']) ? json_decode($rows_rings['Images']) : json_decode($rows_rings['GroupImages']) );
                    
                    $formid = 'form'.$r;
                    $buynow = 'buy'.$r;
                    echo "<div class='col-sm-4'><form id='".$formid."' name='".$formid."' method='post' action=''>";
                    
                    $ringdetail_link = config_item('base_url').'qualitystuller/stullerringsdetail/'.$rows_rings['stuller_id'].'?cate=1';
                    ...
                    $str = 150;
                    $ringBoxDesc = cleanString($rows_rings['Description']);
                    $stullerTitle = ( strlen($ringBoxDesc) > $str ? substr($ringBoxDesc, $str).'...' : $ringBoxDesc );
                    
                    $ringDescription = $sitesTitle.' <br>'.  str_replace(array('"'), ' inch', $rows_rings['Description']);
                        
                    $ringDetaiLink = $ringdetail_link;
                    $buildRing = config_item('base_url').'engagement/search';
                    $wishList  = config_item('base_url').'account/account_wishlist/'.$rows_rings['stuller_id'];
      ...
      <span class="setcolr">&ndash;</span>
        ...
        <div class="price-product ringprice-section">
          ...
          <div><?php echo $rows_rings['Description']; ?></div>
          </div>
      </div>
      <?php 
      echo '</form></div>';
